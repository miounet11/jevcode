---
title: "Pesquisa linha a linha"
description: "Construa uma busca semântica para os Termos de Serviço do GitHub. Em uma única solicitação, pontue 218 identificadores de linha contra uma consulta em linguagem simples com uma pergunta do tipo Escolha e utilize uma pergunta do tipo Noul para verificar se o documento contém uma resposta."
section: cases
order: 260
...
translatedFrom: en
---
Você tem os Termos de Serviço do GitHub e uma pergunta em linguagem simples sobre eles. Você precisa das linhas que respondem à pergunta e de uma maneira de detectar quando o documento não tem resposta. As consultas incluídas classificam as linhas com respostas diretas primeiro. Os limiares `exists` classificam os casos restantes como ausentes ou parciais. Você termina com `find()`, que retorna a probabilidade `exists` e uma pontuação de relevância por linha.

<img src="/img/cases/semantic-find-semantic-find-hero.gif"
 alt="Uma consulta analisa um documento e revela uma resposta anexada à linha correspondente"
 width="1120"
 height="520"
 data-path="cookbooks/semantic_find/semantic-find-hero.gif"
/>

O backend de busca se completa em três partes:

1. Etiquete cada linha com um ID para que o TypeSafe possa apontar para ela.
2. Use uma pergunta `Choice` para classificar esses IDs de linha por quão bem eles respondem à consulta. As probabilidades da pergunta de escolha sempre somam 1, então uma linha é classificada em primeiro lugar mesmo quando nenhuma responde à consulta.
3. Na mesma solicitação, use uma pergunta `Noul` para verificar se o documento contém uma resposta.
...
`JsonCache` reproduz as respostas da API incluídas, para que as etapas abaixo sejam executadas sem uma chave de API
ou qualquer gasto. Para tornar as solicitações reais, defina `TYPESAFE_API_KEY` e exclua
`json_cache.json`.
...
Agora, adicione um ID curto no início de cada linha e junte as linhas de volta em um único documento. O modelo usa esses IDs para apontar para sua resposta.
...
Uma `Choice` pergunta retorna uma probabilidade para cada opção. Use os IDs das linhas como as
opções,
...
As descrições das opções são `None` porque o documento já contém o texto para cada ID. A consulta vai em `instructions`; o estado permanece inalterado entre as buscas.


> **Nota** — Uma pergunta `Choice` aceita até 255 opções, portanto esta receita pesquisa documentos de até 255 linhas em uma única solicitação. Além disso, pesquise em duas etapas: uma pergunta Choice seleciona uma janela de linhas e uma segunda etapa classifica as linhas dentro dela.
...
Então faça uma segunda pergunta, na mesma solicitação:
...
Ao contrário das probabilidades do Choice, a probabilidade do Noul não depende das outras opções,
por isso pode cair perto de zero quando o documento não tem resposta.

## Passo 4: envie ambas as perguntas em uma única solicitação

O método `system_one` responde a ambas as perguntas em uma única passagem. O estado é enviado uma vez, portanto, adicionar a verificação de existência requer apenas uma pequena quantidade de saída adicional.

<img src="/img/cases/semantic-find-recipe.png"
 alt="Um documento marcado e a pergunta do usuário entram em uma solicitação TypeSafe. Uma pergunta Choice pontua
...
Duas peças de código local concluem o trabalho: `verdict()` converte a probabilidade bruta `exists` em três estados, com um estado intermediário para respostas parciais, e `show()` renderiza `relevance` como um gráfico de barras para que a classificação seja legível em um terminal.
...
Esses limiares separam os exemplos abaixo, mas ajuste-os em relação aos seus próprios documentos
antes de utilizá-los em produção.
...
* **Arbitragem:** A classificação dá à linha mais próxima uma pontuação de 0,86, mas `exists` tem apenas 0,14. A resposta não está no documento.
...
O ranking indica onde procurar; a pontuação ⦇0⦇ informa se o resultado responde à pergunta.

## Experimente no seu próprio documento

[Abra o contrato sinalizado no playground do TypeSafe][playground] para editar as perguntas
contra o mesmo texto. Para buscar os seus próprios, substitua a URL em ⦇0⦇; cada outra
linha do script funciona com base em ⦇1⦇.
