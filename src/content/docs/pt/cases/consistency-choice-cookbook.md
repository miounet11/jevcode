---
title: "Autoconsistência: escolhas"
description: "Adicione um resultado incerto às decisões de moderação e compare a concordância de rótulos com a proporção de ações automáticas."
section: cases
order: 150
...
Os LLMs recebem `json.dumps(POST)` no prompt. O TypeSafe recebe o dicionário Python diretamente.
...
Nesta execução `typesafe_choice` possui uma latência média de ida e volta de 114ms. As condições do LLM variam de 826ms a 13,0 segundos por chamada sob as configurações de concorrência acima.

## Enredo: a decisão de cada amostra como um mapa de calor
