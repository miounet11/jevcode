---
title: "Cascata SDE"
description: "Utiliza um cascata de extração de dados estruturados em duas etapas (mini → verificar → raciocínio) para obter a maior parte da qualidade de um modelo de raciocínio grande por uma fração do custo."
section: cases
order: 250
...
---
* Visão geral
 * grandes modelos de raciocínio extraem dados estruturados bem, mas são lentos e caros
 * modelos pequenos são baratos, mas cometem erros
 * uma *cascata* obtém a maior parte da qualidade por uma fração do custo
 * os modelos que usamos, e seus preços (\$ por 1M de tokens, entrada / saída; tarifas padrão
 verificadas em 15 de setembro de 2026):
 * degrau 0 (mini): [`gpt-5.4-mini`](https://developers.openai.com/api/docs/models/gpt-5.4-mini)
 a \$0,75 / \$4,50
 * degrau 1 (raciocínio): [`gpt-5.5`](https://developers.openai.com/api/docs/models/gpt-5.5)
 a \$5,00 / \$30,00 (aproximadamente 7x o mini)
 * verificador: TypeSafe `jev-1.12` a \$0,042 / \$0,00 (os tokens de saída são gratuitos;
 [preços publicados da Jev](https://typesafe.ai/blog/introducing-system-one-models-and-jev))
* Algoritmo
 1. **Extrair** com um modelo barato/pequeno.
 2. **Verificar** com primitivas **TypeSafe**: uma pergunta sim/não por campo ("pergunta Noul")
 * (por exemplo, "este valor está ausente na fonte?", "foi extraído de texto não relacionado?"),
 cada uma retornando P(algo está errado).
 3. **Escalar** para um modelo de raciocínio caro se um sinal do verificador for acionado; caso contrário,
 manter a resposta barata.
* Este Cookbook
 * percorre um exemplo real do início ao fim, e depois mostra a compensação em 100 prompts
 * nota: os dois degraus de extração usam o modo de texto da OpenAI
 * não *usamos* saídas estruturadas, chamadas de ferramentas ou modo json, porque:
 * um erro de *seguimento de esquema* não é o erro que esperamos que um LLM cometa (é fácil
 criar dados sintéticos para isso)
 * se um LLM falhar ao seguir o esquema, quase sempre está muito confuso, então a decodificação
 restrita não corrige o problema subjacente
 * encorajamos você a tentar, no entanto!

## Configuração

* instalar as dependências (o cliente verificador do TypeSafe é servido a partir do índice de pacotes do TypeSafe):

```bash
pip install openai datasets jsonschema ipython "typesafe-sdk>=0.5.7" cooksafe --extra-index-url https://pypi.typesafe.ai/
```

* defina ⦇0⦇ e ⦇1⦇ no seu ambiente
...
Escolhemos um dataset do Hugging Face chamado scrapegraphai
 ...
 * o raspado do prompt capturou apenas a navegação do calendário e o texto padrão: **não há
 data de inscrição, nem descrição**
 * observe que o campo `description` do esquema já envia um valor de *exemplo* ("As inscrições para o semestre de outono abrem") na própria descrição do campo
* então, um extrator bem-comportado deve *recusar* inventar os campos que a página não
 contém
* vamos ver se o modelo pequeno faz a coisa certa!

## Passo 2: extrair com o modelo mini (modo texto)

* nota: `gpt-5.4-mini` é muito estocástico nesta entrada -- mesmo em `temperature=0` ele
 inventa um `description` diferente em quase todas as execuções. Para um walkthrough reproduzível, nós
 **codificamos** a única fabricação canônica que o restante deste notebook explica (e que
 o verificador sinaliza em P(errado) > 0,8). Um pipeline real simplesmente pegaria `extract(MINI,
 prompt, schema, content, temperature=0)` diretamente.
...
* O registro é **válido pelo esquema** (a linha acima imprime `True`), mas está errado:
 * `registration_open_date` fica em branco, o que corresponde à página: ela afirma que não há data
 * mas `description` é fabricado: a página nunca descreve uma data de registro, então
 o mini inventa uma plausível. Pode repetir o próprio exemplo do esquema, "O registro
 abre para o semestre de outono", ou narrar "...não foi encontrado no documento"
 * uma verificação do JSON-Schema não consegue detectar isso. Um modelo barato produz fabricações confiantes
 que satisfazem o esquema desse tipo, e a tarefa de detectá-las cabe a um
...
* TypeSafe retorna uma `noul` calibrada = `P(true)` por pergunta, em uma única chamada system\_one
* o conjunto de perguntas:
 * uma cabeça holística **`__overall__::judge`** principal ("este registro deve ser escalado?"). Nós
 a calculamos e exibimos para contrastar um julgamento do registro inteiro com as cabeças por campo,
 mas a porta na Etapa 4 **não** a utiliza -- a escalada é conduzida pela bateria por campo.
 ...
 * (o pipeline completo também possui uma cabeça `spurious` para contêineres inteiros e uma pontuação geral
 `difficulty`; não mostrada aqui, para manter este roteiro nas duas cabeças de gateamento)
* **A Maneira TypeSafe: Decomposição**
 * Observe como tudo é *programaticamente decomposto*, esta é a maneira TypeSafe.
 * A decomposição maximiza a inteligência de cada prompt e torna o algoritmo
 ajustável e interpretável.
...
### Execute toda a bateria sobre a mini extração
...
Como um sinal foi acionado, pagamos pelo modelo forte (`gpt-5.5`, `reasoning_effort="high"`)
 ...
 * E gastou apenas dólares do modelo de raciocínio neste único item *porque o verificador disse
 a ele para fazê-lo*
 ...
 * o mesmo loop `extract → verify → escalate`, `gpt-5.4-mini → gpt-5.5-reasoning`,
 com o gate `any_flag` sobre os cabeçalhos por campo, executado em 100 prompts do scrapegraphai
 * a extração de baixo custo de cada item é pontuada pelo TypeSafe; o limiar do gate ("cut") é
 varrido de 0→1, e cada configuração resultante é plotada no espaço (custo, qualidade)
 * o gráfico é uma captura histórica; seus custos não foram recalculados à
 taxa atual do Jev listada acima
...
* como ler:
 * **losangos pretos** = os quatro modelos executados isoladamente (o custo sobe com a capacidade; o
 mais forte, `gpt-5.5-reasoning`, está no canto superior direito com ≈0.81 de qualidade por ≈\$0,10/extração)
 * **pontos azuis** = a cascata em vários limiares de filtro; a linha tracejada é a **fronteira de Pareto**
 * a fronteira da cascata fica **acima e à esquerda de cada modelo individual**: ajustar o filtro compra
 a maior parte da qualidade do modelo principal por uma fração do seu custo
 * o degrau mais barato cuida dos itens fáceis por quase nada, e apenas os itens sinalizados pagam pelo
 modelo de raciocínio

## Apêndice A: o que torna um sinal de verificador bom

* a cascata é tão boa quanto seu verificador; o que separa um sinal útil de
 um inútil:
 * **Estreito e fundamentado.**
 * uma verificação sim/não sobre um campo em relação à fonte (ex.: "este valor está ausente
 na fonte?"), não uma pergunta vaga como "esta extração é boa?"
 * perguntas vagas geram escores nebulosos e não calibrados
 * **Ruim = TRUE, com critérios explícitos.**
 * formule cada pergunta de modo que o caso de *escalar* seja o caso `true`, e declare o que
 `true`/`false` significam
 * **Por campo, depois agregue com `max`.**
 * um sinalizador por campo localiza o erro e permanece esparsos e fortes
 * `max` ("qualquer sinalizador dispara") garante que um único sinalizador vermelho confiante seja escalonado, em vez de ser
 diluído no silêncio
 * **Independente e barato.**
 * um verificador dedicado (aqui, TypeSafe) que avalia a saída captura os próprios pontos cegos do extrator
 * precisa ser barato, caso contrário, não haverá economias restantes a capturar
 * **Separado / calibrado.**
 * um bom sinal é alto em erros reais e baixo em acertos, de modo que um único limiar
 separe claramente aceitar vs. escalar
