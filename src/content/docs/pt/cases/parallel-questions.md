---
title: "Perguntas paralelas"
description: "Executa um briefing regulatório de 13 perguntas sobre o artigo da Wikipedia sobre o GDPR, demonstrando que agrupar todas as perguntas em uma única chamada TypeSafe é 12,2 vezes mais barato e 10,0 vezes mais rápido, sem alteração nas respostas."
section: cases
order: 220
...
translatedFrom: en
---
Você tem um documento e N perguntas sobre ele. Você pode enviar uma única solicitação com todas as N perguntas, ou N solicitações com uma pergunta cada. Com o TypeSafe, as respostas saem iguais de qualquer forma: cada pergunta é avaliada individualmente contra o documento, então sua resposta não depende do que mais está na solicitação.

Para verificar isso, o livro de receitas faz cada pergunta várias vezes em ambos os sentidos — todos os N em uma
solicitação, e uma pergunta por solicitação — e compara o desvio padrão entre execuções: o quanto uma
resposta se afasta de uma repetição para a próxima. Qualquer ruído que uma pergunta tenha, ele está presente sob
ambas as estratégias de agrupamento. O agrupamento não adiciona nenhum. A maioria das respostas voltou idêntica em todas
as 5 repetições de qualquer maneira, o mesmo valor em cada chamada, desvio padrão exatamente 0.0.

Custo e velocidade mudam. O documento domina cada solicitação. N chamadas de pergunta única pagam por ele N vezes, em N viagens de ida e volta; a chamada em lote paga uma vez. Quanto maior o documento, mais próxima essa economia chega de um N×.
...
solicitação), e uma equipe de conformidade deseja verificar 13 coisas: 8 `Noul` perguntas, 2 `Choice`
...
## O documento: o artigo da Wikipédia sobre o RGPD

Obtido como texto puro de uma revisão fixa do artigo e armazenado em cache em `json_cache.json`
...
📄 [Leia a revisão fixada da Wikipedia](https://en.wikipedia.org/?oldid=1363040264)

## As perguntas: 8 nouls + 2 choices + 3 scores

Um número rastreado por resposta, por tipo:

* `Noul`: a probabilidade de "sim".
* `Choice`: a probabilidade máxima, a probabilidade do rótulo escolhido. `criteria` mapeia cada
 rótulo ao seu significado.
* `Score`: a pontuação normalizada para 0-1, a pontuação dividida pelo nível superior.
 `criteria` lista as descrições dos níveis, do nível 0 para cima.
...
`ask()` envia qualquer subconjunto das perguntas junto com o documento e reduz cada resposta ao seu
número rastreado único. O documento é byte-identico em todas as chamadas.

Ambas as estratégias de agrupamento executam `RUNS` = 5 vezes, fornecendo a cada pergunta 5 respostas por estratégia,
suficiente para comparar a média (os dois concordam?) e o desvio padrão (o agrupamento adiciona ruído?).
As chamadas são armazenadas em cache em `json_cache.json`, que acompanha o livro de receitas, portanto, o novo processamento é
...
Por pergunta: a média e o desvio padrão do número rastreado nas 5 execuções, sob cada estratégia de agrupamento. Se o agrupamento alterasse as respostas, as colunas agrupadas difeririam das colunas individuais. Uma média deslocada é viés. Um desvio padrão maior é ruído.
...
* Choices, scores e seis dos oito nouls retornam idênticos nas 5 repetições:
 desvio padrão exatamente 0,0 sob ambas as estratégias de agrupamento, cada lote e chamada única
 retornando o mesmo número. Uma chamada com N perguntas dá as mesmas respostas que N chamadas
 com uma pergunta cada.
* `breach_72h` e `criminal_penalties` apresentam um pequeno ruído de amostragem entre execuções, e é
 do mesmo tamanho sob ambas as estratégias de agrupamento, com as médias concordando dentro desse
 ruído. O ruído é uma propriedade da pergunta, não de como você agrupa: agrupar nem
 desloca a resposta nem adiciona variância.

De qualquer forma, não há efeito de loteamento: a resposta de nenhuma pergunta depende das outras 12 perguntas que compartilham sua solicitação.

## A única diferença: custo e velocidade

Mesmas respostas, conta diferente. O artigo de \~54.000 caracteres domina cada solicitação, então:

* Custo: as 13 chamadas de pergunta única reenviam o artigo 13 vezes; a chamada em lote o envia
 uma
 vez. Essa economia se mantém independentemente de como você executa as chamadas.
* Velocidade: a figura soma as latências das 13 chamadas individuais, portanto, assume que elas são executadas uma após
 a outra. Execute-as em paralelo e a diferença diminui, mas o custo de 13x em tokens permanece.
...
O mesmo artigo e as mesmas 13 perguntas, compactados em um link compartilhável. Abra-o para executar o briefing ao vivo novamente; os mesmos números retornam.
