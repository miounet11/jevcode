---
title: "层级分类"
description: "通过并行束搜索（beam search）在 TypeSafe Choice 概率上进行深层专利、零售产品、生物医学和源代码层级分类。"
section: cases
order: 200
...
许多数据以结构化层次结构的形式存在，例如分类法、文件系统层次结构、网站结构、代码库、组织架构图、生物本体论、LLM 技能、审核策略等。层次分类的目标是遍历层次结构以到达正确的叶节点，该叶节点即为最终分类。这非常适合类型安全系统的 `Choice` 原语。我们通过在每个节点（从根节点开始）对文档进行分类来找到最可能的叶节点，然后迭代地移动到下一个最可能的节点，直到到达叶节点（**贪婪搜索**）。

API 的并行特性还允许我们使用 **束搜索** 通过并行问题探索多条路径，从而提高性能。Cookbook 中的 TypeSafe API 调用会同时评估层次结构的 `K` 条路径。束搜索通过几何平均边概率保留最佳的 `K` 条路径：`product(edge_probabilities) ** (1 / decisions)`，并修剪其余路径。概率经过长度归一化处理，以便公平地比较浅层和深层的叶节点。

像这样将问题分解为层次结构具有其自身的优势：

* 可观测性
  * 识别误分类最常发生的节点
  * 测量每个节点和边被遍历的次数
* 可测试性
  * 单元测试并衡量层次结构更新对分类性能的影响
...
* **[CPC 2026.05](https://www.cooperativepatentclassification.org/sites/default/files/cpc/bulk/CPCSchemeXML202605.zip)：** 专利主题，从广泛的技术部分到狭窄的发明。
* **[Shopify 2026-02](https://github.com/Shopify/product-taxonomy/blob/v2026-02/dist/en/categories.txt)：** 零售产品类别，从商店部门到特定产品类型。
* **[MeSH
  2026](https://nlmpubs.nlm.nih.gov/projects/mesh/MESH_FILES/xmlmesh/desc2026.zip)：**
  生物医学主题，从广泛领域到特定状况。MeSH 是一个有向无环图（DAG），因此一个描述符可以出现在多个父节点下；此演示扩展了其官方树编号路径。
* **CookSafe 文件：** TypeSafe 的 cookbook 仓库层次结构，从文件夹搜索到源文件。

### 方法

* **贪婪搜索：** 选择概率最高的子节点并丢弃所有其他选项。早期的错误无法恢复。
* **束搜索：** 保留 `K` 条合理的路径，并并行分类每个前沿节点。更深层的证据可以修复早期的模糊决策。具有最高几何平均概率的路径的叶节点即为最终分类。
* **TypeSafe Choice：** 每个节点都是一个 `Choice` 问题，其完整的概率分布即为其边。束的每条路径作为并行问题运行，因此额外的探索几乎不会增加墙钟时间延迟。
* **公式：**
  * `path_score = product(edge_probabilities) ** (1 / decisions)`
    * 用于修剪和比较路径
  * `separation = top_path_score / second_path_score`
    * 有用的指标，但不用于修剪
    * 该比率将顶部路径的几何平均值与其最接近的竞争者进行比较。
      * 接近 `1×` 表示模糊
      * 较大的比率表示明显的分离。
* **关于指标的说明：**
  * 不同的指标，如 `min(top_prob/second_top_prob)`，将优化那些在每个节点上都有非常明确决策的路径。
  * 使用 `exp(mean(log(probs)))` 代替 `product(edge_probabilities) ** (1 / decisions)`，以避免对于非常深的层次结构（例如 >10 层）产生的精度误差。

## 加载和可视化示例层次结构

这些辅助函数下载固定版本的分类法源，将其解析为直接子节点树，并将每次搜索遍历渲染为静态 SVG。
...
每个兄弟节点集合在下一节中将成为一个 `Choice` 问题，该部分还将实现两种遍历策略，并保留静态图表所需的概率。
...
在四个已标注的示例上运行这两种策略，将它们的叶子节点与预期分类进行比较，并可视化它们所探索的路径。
...
每个示例都有一个已知的预期叶节点。束搜索（Beam search）匹配了 4 个预期叶节点中的 4 个；贪心搜索（Greedy search）匹配了 4 个预期叶节点中的 2 个。保留三条路径成功恢复了 CPC 专利和 Shopify 产品的预期分类。

| 层级结构             | 预期叶节点                                                                 | 贪心叶节点                                                                 | 束搜索 K=3 叶节点                                                          | 贪心正确 | 束搜索正确 |
| -------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------- | -------- | ---------- |
| CPC 专利             | A01K31/12 家禽或鸟类的栖木，例如栖架                                       | E99Z99/00 本节未另行规定的事项                                             | A01K31/12 家禽或鸟类的栖木，例如栖架                                       | 否       | 是         |
| Shopify 产品         | Cat Window Beds & Perches                                                  | Pet Chairs                                                                 | Cat Window Beds & Perches                                                  | 否       | 是         |
| MeSH 生物医学主题词  | C06.405.469.432.500 克罗恩病                                               | C06.405.469.432.500 克罗恩病                                               | C06.405.469.432.500 克罗恩病                                               | 是       | 是         |
| CookSafe 文件        | retrievers.py                                                              | retrievers.py                                                              | retrievers.py                                                              | 是       | 是         |

这些图表展示了不同方法之间的差异。橙色标记了贪心路径，绿色标记了获胜的束路径，紫色标记了其他保留的路径，虚线边表示被剪枝。
