---
title: "并行问题"
description: "在 GDPR 维基百科文章上运行 13 个问题的监管简报，结果表明，将所有问题批量处理到一个 TypeSafe 调用中，成本降低 12.2 倍，速度提升 10.0 倍，且答案无任何变化。"
section: cases
order: 220
...
📄 [查看置顶的维基百科修订版](https://en.wikipedia.org/?oldid=1363040264)
...
* 选项、得分以及八个 Noul 中的六个在 5 次重复中完全一致：
  在两种批处理策略下，标准差均为 0.0，每次批处理和单次调用返回的数字都相同。一次调用包含 N 个问题与 N 次调用各包含一个问题，所得答案相同。
