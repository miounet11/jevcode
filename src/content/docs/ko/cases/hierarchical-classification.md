---
title: "계층적 분류"
description: "TypeSafe Choice 확률에 대한 병렬 빔 검색을 통해 심층 특허, 소매 제품, 생물의학 및 소스 코드 계층 구조를 사용하여 문서를 분류합니다."
section: cases
order: 200
...
translatedFrom: en
---
많은 데이터는 분류체계, 파일시스템 계층, 웹사이트 구조, 코드베이스, 조직도, 생물학적 오토로직, LLM 스킬, 모더레이션 정책 등과 같은 구조화된 계층 형태로 존재합니다. 계층적 분류(Hierarchical Classification)의 목표는 올바른 리프 노드까지 계층을 탐색하여 최종 분류를 결정하는 것입니다. 이는 TypeSafe의 `Choice` 원리에 완벽하게 부합합니다. 우리는 루트에서 시작하여 각 노드에서 문서를 분류함으로써 가장 확률이 높은 리프를 찾아내고, 리프에 도달할 때까지 가장 확률이 높은 다음 노드로 반복적으로 이동합니다(**Greedy Search**).

API의 병렬 특성은 **빔 서치(Beam Search)**를 사용하여 여러 경로를 병렬 질문으로 탐색함으로써 성능을 향상시킬 수 있게 합니다. 이 쿡북의 TypeSafe API 호출은 계층의 `K` 경로를 동시에 평가합니다. 빔 서치는 기하평균 에지 확률 `product(edge_probabilities) ** (1 / decisions)`에 따라 가장 좋은 `K` 경로를 유지하고 나머지는 가지치기합니다. 이 확률은 길이가 정규화되어 있으므로, 얕은 리프와 깊은 리프를 공정하게 비교할 수 있습니다.

이러한 계층 구조로 문제를 분해하는 것에도 자체적인 이점이 있습니다:

* 관찰 가능성
 * 오분류가 가장 많이 발생하는 노드를 식별
 * 각 노드와 엣지가 횡수되는 횟수를 측정
* 테스트 가능성
 * 계층 구조 업데이트가 분류 성능에 미치는 영향을 단위 테스트하고 측정
...
* **[CPC 2026.05](https://www.cooperativepatentclassification.org/sites/default/files/cpc/bulk/CPCSchemeXML202605.zip):** 특허 대상, 광범위한 기술 섹션부터 구체적인 발명까지.
* **[Shopify 2026-02](https://github.com/Shopify/product-taxonomy/blob/v2026-02/dist/en/categories.txt):** 소매 제품 카테고리, 매장 부서부터 특정 제품 유형까지.
* **[MeSH
 2026](https://nlmpubs.nlm.nih.gov/projects/mesh/MESH_FILES/xmlmesh/desc2026.zip):**
 광범위한 도메인부터 특정 질환까지의 생물의학 주제. MeSH는 DAG이므로 하나의
 설명자가 여러 부모 아래에 나타날 수 있습니다. 이 데모는 공식 트리 번호
 경로를 확장합니다.
* **CookSafe 파일:** TypeSafe의 요리책 저장소 계층 구조, 폴더부터
 소스 파일까지 검색됨.

### 메서드

* **탐욕적 검색:** 가장 확률이 높은 자손을 선택하고 모든 대안을 폐기합니다.
 초기의 실한 번은 복구할 수 없습니다.
* **빔 검색:** `K` 개의 타당한 경로를 유지하고 프론티어(경계)의 모든 항목을 병렬로 분류합니다.
 더 깊은 증거는 모호한 초기 결정을 수정할 수 있습니다. 기하평균 확률이 가장 높은 경로의 리프가 최종 분류가 됩니다.
* **TypeSafe Choice:** 모든 노드는 `Choice` 질문이며, 그 전체 확률 분포가 간선입니다. 빔의 각 경로는 병렬 질문으로 실행되므로, 추가 탐색은 실제 실행 시간(latency)에 거의 영향을 미치지 않습니다.
* **공식:**
 * `path_score = product(edge_probabilities) ** (1 / decisions)`
 * 경로 가지치기 및 경로 비교에 사용됨
 * `separation = top_path_score / second_path_score`
 * 유용한 지표이지만 가지치기에는 사용되지 않음
 * 이 비율은 상위 경로의 기하평균을 가장 가까운 경쟁자와 비교합니다.
 * `1×` 근처는 모호함
 * 큰 비율은 명확한 분리를 의미합니다.
* **지표에 대한 참고 사항:**
 * 각 노드에서 매우 명확한 결정을 내리는 경로를 최적화하도록 설계된 `min(top_prob/second_top_prob)`와 같은 다른 지표
 * 매우 깊은 계층(예: >10층)의 정밀도 오류를 피하기 위해 `product(edge_probabilities) ** (1 / decisions)` 대신 `exp(mean(log(probs)))` 사용

## 예제 계층 구조 로드 및 시각화

이 헬퍼들은 고정된 분류원소를 다운로드하고, 이를 직계 자식 트리로 파싱하며, 각 검색 트래버설을 정적 SVG로 렌더링합니다.
...
각 형제 집합은 다음 섹션에서 하나의 `Choice` 질문이 되며, 여기서는 또한 두 가지 탐색 전략을 구현하고 정적 다이어그램이 필요한 확률을 고정합니다.
...
두 가지 전략을 네 개의 레이블이 지정된 예시에 대해 실행하고, 그 리프 노드를 예상되는 분류와 비교하며 탐색한 경로를 시각화하세요.
...
각 예시에는 알려진 최종 리프가 있습니다. 빔 검색은 4개의 예상 리프 중 4개를 일치시켰으며, 탐욕 검색은 4개 중 2개를 일치시켰습니다. 세 개의 경로를 유지함으로써 CPC 특허와 Shopify 제품의 예상 분류를 복원했습니다.

| 계층 구조 | 예상 리프 | 탐욕 리프 | 빔 K=3 리프 | 탐욕 정확 | 빔 정확 |
| ------------------------ | ------------------------------------------------------- | ------------------------------------------------------------------------- | --------------------------------------------------------- | -------- | ------ |
| CPC 특허 | A01K31/12 가금류 또는 조류용 지주, 예: 횃대 | E99Z99/00 이 섹션에서 다른 곳으로 제공되지 않은 주제 | A01K31/12 가금류 또는 조류용 지주, 예: 횃대 | 아니요 | 예 |
| Shopify 제품 | 고양이 창문 침대 & 지주 | 애완동물 의자 | 고양이 창문 침대 & 지주 | 아니요 | 예 |
| MeSH 생물 의학 주제 | C06.405.469.432.500 크론병 | C06.405.469.432.500 크론병 | C06.405.469.432.500 크론병 | 예 | 예 |
| CookSafe 파일 | retrievers.py | retrievers.py | retrievers.py | 예 | 예 |

다이어그램은 방법들이 어떻게 다른지를 보여줍니다. 주황색은 탐욕 경로를, 초록색은 승리한 빔 경로를, 보라색은 다른 유지된 경로를 표시하며, 점선 간선은 가지치기되었습니다.
