---
title: "줄별 검색"
description: "GitHub의 이용약관에 대한 의미론적 검색을 구축합니다. 단일 요청에서 Choice 질문을 사용하여 평이한 언어의 쿼리에 대해 218개의 라인 ID를 점수화하고, Noul 질문을 사용하여 해당 문서에 답변이 포함되어 있는지 확인합니다."
section: cases
order: 260
...
<img src="/img/cases/semantic-find-semantic-find-hero.gif"
 alt="쿼리가 문서를 스캔하고 일치하는 행에 연결된 답변을 표시함"
 width="1120"
 height="520"
...
1. TypeSafe가 각 줄을 가리킬 수 있도록 줄마다 ID를 붙입니다.
2. `Choice` 질문을 사용하여, 그 줄 ID들을 쿼리에 얼마나 잘 답하는지에 따라 순위를 매깁니다. Choice 질문의 확률은 항상 1로 합산되므로, 쿼리에 답하는 줄이 없더라도 한 줄이 1위를 차지합니다.
3. 동일한 요청에서 `Noul` 질문을 사용하여 문서에 답이 포함되어 있는지 확인합니다.
...
실제 요청을 수행하려면 `TYPESAFE_API_KEY`을 설정하고 `json_cache.json`을 삭제하세요.

### 스크립트 생성

`semantic_search.py`에서 import와 클라이언트를 시작하세요:
...
옵션 설명은 `None`입니다. 문서에는 각 ID에 해당하는 텍스트가 이미 포함되어 있기 때문입니다. 쿼리는 `instructions`에 입력되며, 검색 간 상태는 변경되지 않습니다.


> **참고** — `Choice` 질문은 최대 255개의 옵션을 허용하므로, 이 레시피는 한 번의 요청으로 최대 255줄의 문서를 검색합니다. 이를 초과할 경우 두 번의 단계로 검색합니다: 첫 번째 Choice 질문은 줄의 범위를 선택하고, 두 번째 단계는 해당 범위 내의 줄을 순위 매깁니다.
...
<img src="/img/cases/semantic-find-recipe.png"
 alt="태그가 지정된 문서와 사용자 질문이 하나의 TypeSafe 요청에 입력됩니다. Choice 질문은 모든 줄을 채점하고, Noul 질문은 답변이 존재하는지 확인합니다. 로컬 코드는 줄을 순위 매기고 문서의 심판을 적용합니다."
 width="2170"
 height="458"
...
두 가지 로컬 코드가 작업을 마무리합니다: `verdict()`는 원시 `exists` 확률을 세 가지 상태로 변환하며, 부분 정답을 위한 중간 상태도 포함됩니다. `show()`는 `relevance`를 막대 차트로 렌더링하여 터미널에서 순위를 쉽게 읽을 수 있게 합니다.
...
* **중재:** 랭킹은 가장 가까운 줄에 0.86점을 부여하지만, `exists`는 0.14에 불과하다. 답변은 문서에 없다.
* **부모 동의:** 연령 규정이 1위를 차지하지만, 부모 동의가 규정을 변경하는지 여부는 답변하지 않는다. 결과는 **부분적으로 다뤄짐**.

랭킹은 어디를 봐야 하는지 알려주고, `exists` 점수는 결과가 질문을 답변하는지 알려줍니다.

## 자신의 문서에서 사용해 보세요

[TypeSafe 플레이그라운드에서 태그된 계약서 열기][playground]하여 동일한 텍스트에 대한 질문을 수정하세요. 사용자 고유의 데이터를 검색하려면 `fetch_document()`의 URL을 교체하세요; 스크립트의 나머지 모든 줄은 `LINES`을 기준으로 작동합니다.
