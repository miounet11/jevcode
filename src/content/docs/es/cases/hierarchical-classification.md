...
description: "Clasifica documentos mediante jerarquías profundas de patentes, productos minoristas, biomedicina y código fuente, utilizando búsqueda en haz paralela sobre probabilidades de elección TypeSafe."
section: cases
order: 200
...
translatedFrom: en
---
Existe una gran cantidad de datos en forma de jerarquías estructuradas, como taxonomías, jerarquías de sistemas de archivos, estructuras de sitios web, repositorios de código, organigramas, ontologías biológicas, habilidades de LLM, políticas de moderación, etc. El objetivo de la Clasificación Jerárquica es recorrer la jerarquía hasta llegar al nodo hoja correcto, que es la clasificación final. Esto encaja perfectamente con la primitiva `Choice` de TypeSafe. Encontramos la hoja más probable clasificando el documento en cada nodo (comenzando por la raíz) y luego avanzamos iterativamente hacia el siguiente nodo más probable hasta llegar a una hoja (**Búsqueda Greedy**).

La naturaleza paralela de la API también nos permite explorar múltiples caminos con preguntas paralelas
utilizando **Beam Search** para mejorar el rendimiento. Las llamadas a la API TypeSafe del libro de recetas evalúan cada una
simultáneamente `K` caminos de la jerarquía. La búsqueda en haz mantiene los mejores `K` caminos
por una probabilidad de arista media geométrica: `product(edge_probabilities) ** (1 / decisions)`,
y poda el resto. La probabilidad está normalizada por longitud para que las hojas superficiales y profundas
se comparen de manera justa.

Descomponer el problema en una jerarquía como esta tiene sus propios beneficios:

* Observabilidad
 * identificar en qué nodos ocurren más tus clasificaciones erróneas
 * medir el número de veces que se atraviesa cada nodo y arista
* Testabilidad
 * realizar pruebas unitarias y medir el impacto de las actualizaciones de jerarquía en el rendimiento de la clasificación
...
### Jerarquías utilizadas en este cookbook

* **[CPC 2026.05](https://www.cooperativepatentclassification.org/sites/default/files/cpc/bulk/CPCSchemeXML202605.zip):** materia de patentes, desde secciones tecnológicas amplias hasta invenciones específicas.
* **[Shopify 2026-02](https://github.com/Shopify/product-taxonomy/blob/v2026-02/dist/en/categories.txt):** categorías de productos minoristas, desde departamentos de tiendas hasta tipos de productos específicos.
* **[MeSH
 2026](https://nlmpubs.nlm.nih.gov/projects/mesh/MESH_FILES/xmlmesh/desc2026.zip):**
 temas biomédicos desde dominios amplios hasta condiciones específicas. MeSH es un DAG, por lo que un
 descriptor puede aparecer bajo múltiples padres; esta demo expande sus rutas oficiales de
 números de árbol.
* **Archivos CookSafe:** jerarquía del repositorio de recetas de TypeSafe, buscado desde carpetas hasta
 archivos fuente.

### Métodos

* **Búsqueda voraz:** elige el hijo con mayor probabilidad y descarta todas las alternativas.
 Un error temprano no puede recuperarse.
* **Búsqueda por haces:** conserva `K` caminos plausibles y clasifica cada frontera en paralelo.
 La evidencia más profunda puede reparar una decisión temprana ambigua. La hoja del camino con
 la mayor probabilidad geométrica media es la clasificación final.
* **TypeSafe Choice:** cada nodo es una `Choice` pregunta cuya distribución de probabilidad completa es sus
 aristas. Cada camino del haz se ejecuta como preguntas en paralelo, por lo que la exploración adicional añade poca
 latencia de reloj de pared.
* **Fórmula:**
 * `path_score = product(edge_probabilities) ** (1 / decisions)`
 * utilizada para podar y comparar caminos
 * `separation = top_path_score / second_path_score`
 * métrica útil, pero no se usa para podar
 * la razón compara la media geométrica del camino principal contra su rival más cercano.
 * Cerca de `1×` es ambiguo
 * Una gran razón significa una separación clara.
* **Notas sobre métricas:**
 * una métrica diferente como `min(top_prob/second_top_prob)` que optimizaría para
 caminos que tienen decisiones muy claras en cada nodo.
 * usa `exp(mean(log(probs)))` en lugar de `product(edge_probabilities) ** (1 / decisions)`
 para evitar errores de precisión en jerarquías que son muy profundas (por ejemplo, >10 capas)

## Cargar y visualizar las jerarquías de ejemplo

Estos ayudantes descargan las fuentes de taxonomía fijadas, las analizan en árboles de hijos directos y renderizan cada recorrido de búsqueda como un SVG estático.
...
## Implementar búsqueda voraz y por haz

Cada conjunto de hermanos se convierte en una `Choice` pregunta en la siguiente sección, que también
implementa
ambas estrategias de recorrido y mantiene las probabilidades que los diagramas estáticos necesitan.
...
Ejecuta ambas estrategias en cuatro ejemplos etiquetados, compara sus hojas frente a las clasificaciones esperadas y visualiza las rutas que exploraron.
...
Cada ejemplo tiene una hoja esperada conocida. La búsqueda en haz coincidió 4 de 4 hojas esperadas; la búsqueda codiciosa coincidió 2 de 4. Mantener tres caminos recuperó la clasificación esperada para las patentes CPC, productos Shopify.

| Jerarquía | Hoja esperada | Hoja greedy | Hoja Beam K=3 | Correcto greedy | Correcto beam |
| ------------------------ | --------------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------- | --------------- | ------------- |
| Patentes CPC | A01K31/12 Posaderos para aves de corral o pájaros, p. ej. posaderos | E99Z99/00 Materia no previsto en otra parte de esta sección | A01K31/12 Posaderos para aves de corral o pájaros, p. ej. posaderos | no | sí |
| Productos Shopify | Camas y posaderos para gatos con ventana | Sillas para mascotas | Camas y posaderos para gatos con ventana | no | sí |
| Asuntos biomédicos MeSH | C06.405.469.432.500 Enfermedad de Crohn | C06.405.469.432.500 Enfermedad de Crohn | C06.405.469.432.500 Enfermedad de Crohn | sí | sí |
| Archivos CookSafe | retrievers.py | retrievers.py | retrievers.py | sí | sí |

Los diagramas muestran por qué los métodos difieren. El naranja marca la ruta codiciosa, el verde marca la ruta de haz ganadora, el morado marca otras rutas retenidas y los bordes punteados fueron podados.
