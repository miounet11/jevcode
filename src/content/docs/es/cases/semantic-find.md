---
title: "Búsqueda línea por línea"
description: "Construye una búsqueda semántica para los Términos de servicio de GitHub. En una sola solicitud, evalúa 218 identificadores de línea frente a una consulta en lenguaje sencillo mediante una pregunta de opción múltiple, y utiliza una pregunta de tipo Noul para verificar si el documento contiene una respuesta."
section: cases
order: 260
tags: ['cookbook', 'recipe']
source: "docs.typesafe.ai/cookbooks/semantic_find"
translatedFrom: en
---
Tienes los Términos de servicio de GitHub y una pregunta en lenguaje sencillo sobre ellos. Necesitas las líneas que respondan a la pregunta y una forma de detectar cuándo el documento no tiene respuesta. Las consultas incluidas clasifican las líneas con respuestas directas primero. Los `exists` umbrales clasifican los casos restantes como faltantes o parciales. Terminas con `find()`, que devuelve la probabilidad `exists` y una puntuación de relevancia por línea.

<img src="/img/cases/semantic-find-semantic-find-hero.gif"
 alt="Una consulta escanea un documento y revela una respuesta adjunta a la línea coincidente"
 width="1120"
 height="520"
...
2. Usa una pregunta `Choice` para clasificar esos IDs de línea según lo bien que responden a la consulta. Las probabilidades de la pregunta de Choice siempre suman 1, por lo que una línea se clasifica primero incluso cuando ninguna responde a la consulta.
3. En la misma solicitud, usa una pregunta `Noul` para verificar si el documento contiene una respuesta en absoluto.
...
`JsonCache` reproduce las respuestas de la API incluidas, de modo que los pasos siguientes se ejecuten sin clave de API ni gasto. Para hacer que las solicitudes sean reales, establece `TYPESAFE_API_KEY` y elimina `json_cache.json`.
...
El documento de prueba es los Términos de servicio de GitHub, dividido en 218 cláusulas, por lo que cada resultado de búsqueda apunta a una línea citable.
...
Ahora, anteponga a cada línea un identificador corto y vuelva a unir las líneas en un único documento. El modelo utiliza estos identificadores para referenciar su respuesta.
...
Una pregunta `Choice` devuelve una probabilidad para cada opción. Usa los IDs de línea como las opciones,
y "elige una opción" se convierte en "señala una línea."
...
Las probabilidades de Choice siempre suman 1, por lo que algunas líneas se clasifican primero incluso cuando el documento no responde a la pregunta. La clasificación por sí sola no puede distinguir una respuesta real de la línea irrelevante más cercana.
...
A diferencia de las probabilidades de Choice, la probabilidad de Noul no depende de las otras opciones,
por lo que puede caer cerca de cero cuando el documento no tiene respuesta.

## Paso 4: enviar ambas preguntas en una sola solicitud

El método `system_one` responde a ambas preguntas en un solo paso. El estado se envía una vez, por lo que añadir la comprobación de existencia requiere solo una pequeña cantidad de salida adicional.

<img src="/img/cases/semantic-find-recipe.png"
 alt="Un documento etiquetado y la pregunta del usuario entran en una solicitud TypeSafe. Una pregunta Choice puntúa
cada línea mientras una pregunta Noul verifica si existe una respuesta. El código local luego clasifica las
líneas y aplica el veredicto del documento."
 width="2170"
 height="458"
...
Dos fragmentos de código local completan la tarea: `verdict()` convierte la probabilidad `exists` en bruto en tres estados, con uno intermedio para respuestas parciales, y `show()` renderiza `relevance` como un gráfico de barras para que el ranking sea legible en una terminal.
...
* **Arbitraje:** La clasificación otorga una puntuación de 0.86 a la línea más cercana, pero `exists` es solo 0.14. La respuesta no está en el documento.
* **Permiso de los padres:** La regla de edad ocupa el primer puesto, pero no responde si el permiso de los padres modifica la norma. El resultado es **parcialmente abordado**.

El ranking te indica dónde buscar; la puntuación ⦇0⦇ te dice si el resultado responde a la pregunta.

## Pruébalo en tu propio documento

[Abre el contrato etiquetado en el playground de TypeSafe][playground] para editar las preguntas
contra el mismo texto. Para buscar los tuyos propios, intercambia la URL en `fetch_document()`; cada otra
línea del script funciona con base en `LINES`.
