---
title: "Zeilenweise Suche"
description: "Erstellen Sie eine semantische Suche für die Nutzungsbedingungen von GitHub. Bewerten Sie in einer einzigen Anfrage 218 Zeilen-IDs gegen eine Abfrage in einfacher Sprache mit einer Multiple-Choice-Frage und verwenden Sie eine Noul-Frage, um zu prüfen, ob das Dokument eine Antwort enthält."
section: cases
order: 260
...
<img src="/img/cases/semantic-find-semantic-find-hero.gif"
 alt="Eine Abfrage durchsucht ein Dokument und zeigt eine Antwort, die der passenden
...
Das Such-Backend setzt sich aus drei Teilen zusammen:

1. Markieren Sie jede Zeile mit einer ID, damit TypeSafe sie referenzieren kann.
2. Verwenden Sie eine `Choice`-Frage, um diese Zeilen-IDs danach zu rangieren, wie gut sie die Anfrage beantworten. Choice-Fragen-Wahrscheinlichkeiten summieren sich immer zu 1, sodass eine Zeile auf dem ersten Platz rangiert, selbst wenn keine die Anfrage beantwortet.
3. Verwenden Sie in derselben Anfrage eine `Noul`-Frage, um zu prüfen, ob das Dokument überhaupt eine Antwort enthält.
...
`JsonCache` spielt die enthaltenen API-Antworten ab, sodass die folgenden Schritte ohne API-Schlüssel
oder Kosten ausgeführt werden. Um die Anfragen live zu stellen, setze `TYPESAFE_API_KEY` und lösche
`json_cache.json`.
...
Hinzufügen zu `semantic_search.py`:
...
Der Cache verhindert wiederholte Downloads, und `splitlines()` hinterlässt eine Liste mit 218 Strings.

Fügen Sie nun jeder Zeile eine kurze ID hinzu und verbinden Sie die Zeilen wieder zu einem Dokument. Das Modell verwendet diese IDs, um auf seine Antwort zu verweisen.
...
> **Hinweis** — Eine `Choice`-Frage akzeptiert bis zu 255 Optionen, daher durchsucht dieses Rezept Dokumente mit bis zu 255 Zeilen in einer einzigen Anfrage. Darüber hinaus suchen Sie in zwei Durchgängen: Eine Choice-Frage wählt einen Zeilenbereich aus, und eine zweite rangiert die Zeilen innerhalb dieses Bereichs.


## Schritt 3: Prüfen, ob eine Antwort existiert

Die Wahrscheinlichkeiten der Auswahl addieren sich immer zu 1, sodass einige Zeilenranglisten an erster Stelle stehen, auch wenn das Dokument die Frage nicht beantwortet. Die reine Rangfolge kann keine echte Antwort von der nächstliegenden irrelevanten Zeile unterscheiden.
...
Im Gegensatz zu den Choice-Wahrscheinlichkeiten hängt die Noul-Wahrscheinlichkeit nicht von den anderen Optionen ab,
daher kann sie nahe null fallen, wenn das Dokument keine Antwort enthält.

## Schritt 4: Senden Sie beide Fragen in einer Anfrage

Die `system_one`-Methode beantwortet beide Fragen in einem Durchlauf. Der Zustand wird einmal gesendet, sodass das Hinzufügen der Existenzprüfung nur einen geringen zusätzlichen Ausgabeaufwand erfordert.

<img src="/img/cases/semantic-find-recipe.png"
 alt="Ein getaggtes Dokument und eine Benutzerfrage geben eine einzelne TypeSafe-Anfrage ein. Eine Choice-Frage bewertet jede Zeile, während eine Noul-Frage prüft, ob eine Antwort existiert. Lokaler Code rangiert anschließend die Zeilen und wendet das Dokumenturteil an."
 width="2170"
 height="458"
...
Zwei lokale Code-Schnipsel erledigen die Aufgabe: `verdict()` wandelt die rohe `exists`-Wahrscheinlichkeit in drei Zustände um, wobei ein mittlerer Zustand für partielle Antworten vorgesehen ist, und `show()` stellt `relevance` als Balkendiagramm dar, sodass die Rangfolge in einem Terminal lesbar ist.
...
Diese Schwellenwerte trennen die nachfolgenden Beispiele, stimmen Sie sie jedoch vor dem Einsatz in der Produktion auf Ihre eigenen Dokumente ab.
...
Die ersten beiden Abfragen liefern direkte Antworten und die Quellzeilen, die zur Überprüfung benötigt werden.

Die anderen beiden zeigen, warum die Existenzprüfung wichtig ist:

* **Schiedsverfahren:** Die Rangfolge gibt der nächsten Zeile eine Punktzahl von 0,86, aber `exists` ist nur 0,14. Die Antwort ist nicht im Dokument enthalten.
* **Elternlaubnis:** Die Altersregel rangiert an erster Stelle, beantwortet jedoch nicht, ob die Elternlaubnis die Regel ändert. Das Ergebnis ist **teilweise behandelt**.

Die Rangliste sagt Ihnen, wo Sie suchen müssen; die `exists`-Punktzahl sagt Ihnen, ob das Ergebnis die Frage beantwortet.

## Probieren Sie es mit Ihrem eigenen Dokument

[Öffne den markierten Vertrag im TypeSafe-Playground][playground], um die Fragen gegen denselben Text zu bearbeiten. Um deine eigenen zu durchsuchen, tausche die URL in ⦇0⦇; jede andere Zeile des Skripts arbeitet mit ⦇1⦇.
