...
description: "Klassifiziert Dokumente durch tiefe Hierarchien in den Bereichen Patente, Einzelhandelsprodukte, Biomedizin und Quellcode mittels paralleler Beam-Suche über Wahrscheinlichkeiten der TypeSafe-Choice-Kategorien."
section: cases
order: 200
...
translatedFrom: en
---
Viele Daten liegen als strukturierte Hierarchien vor, etwa Taxonomien,
Dateisystem-Hierarchien, Website-Strukturen, Codebasen, Organigramme, biologische Ontologien, LLM-Fähigkeiten,
Moderationsrichtlinien usw. Das Ziel der Hierarchischen Klassifizierung besteht darin, die
Hierarchie bis zum korrekten Blattknoten zu durchlaufen, der die endgültige Klassifizierung darstellt. Dies ist eine perfekte
Passform für typesafe's `Choice`-Primitive. Wir finden den wahrscheinlichsten Blattknoten, indem wir das
Dokument an jedem Knoten klassifizieren (beginnend an der Wurzel), und fahren dann iterativ mit dem
nächsthäufigsten Knoten fort, bis wir bei einem Blattknoten landen (**Greedy Search**).

Die parallele Natur der API ermöglicht es uns auch, mehrere Pfade mit parallelen Fragen zu erkunden, indem wir **Beam Search** einsetzen, um die Leistung zu verbessern. Die TypeSafe-API-Aufrufe des Kochbuchs bewerten jede `K` Pfade der Hierarchie gleichzeitig aus. Beam Search behält die besten `K` Pfade bei einer geometrischen Mittelkantenwahrscheinlichkeit: `product(edge_probabilities) ** (1 / decisions)`,
und beschneidet den Rest. Die Wahrscheinlichkeit ist längennormalisiert, sodass flache und tiefe Blätter fair verglichen werden.

Die Zerlegung des Problems in eine solche Hierarchie hat ihre eigenen Vorteile:

* Beobachtbarkeit
 * identifizieren, in welchen Knoten Ihre Fehlklassifikationen am häufigsten auftreten
 * die Anzahl der Durchläufe jedes Knotens und jeder Kante messen
* Testbarkeit
 * Komponententests durchführen und die Auswirkungen von Hierarchieaktualisierungen auf die Klassifizierungsleistung messen
...
### Hierarchien, die in diesem Kochbuch verwendet werden

* **[CPC 2026.05](https://www.cooperativepatentclassification.org/sites/default/files/cpc/bulk/CPCSchemeXML202605.zip):** Patentgegenstände, von breiten Technologieabschnitten bis hin zu engen Erfindungen.
* **[Shopify 2026-02](https://github.com/Shopify/product-taxonomy/blob/v2026-02/dist/en/categories.txt):** Einzelhandelsproduktkategorien, von Abteilungen im Geschäft bis zu spezifischen Produkttypen.
* **[MeSH
 2026](https://nlmpubs.nlm.nih.gov/projects/mesh/MESH_FILES/xmlmesh/desc2026.zip):**
 biomedizinische Themen von breiten Domänen bis hin zu spezifischen Erkrankungen. Da MeSH ein DAG ist, kann ein Deskriptor unter mehreren übergeordneten Elementen erscheinen; diese Demo erweitert die offiziellen Tree-Number-Pfade.
* **CookSafe-Dateien:** Die Repository-Hierarchie von TypeSafe’s Kochbuch, durchsucht von Ordnern bis hin zu Quellcodedateien.

### Methoden

* **Gierige Suche:** Wähle das Kind mit der höchsten Wahrscheinlichkeit und verwerfe jede Alternative.
 Ein früher Fehler kann nicht korrigiert werden.
* **Beam Search:** Behalte `K` plausible Pfade und klassifiziere jede Front parallel.
 Tiefer liegende Evidenz kann eine mehrdeutige frühe Entscheidung korrigieren. Der Blattknoten des Pfades mit der
 höchsten geometrischen Mittelwahrscheinlichkeit ist die endgültige Klassifizierung.
* **TypeSafe Choice:** Jeder Knoten ist eine `Choice` Frage, deren vollständige
 Wahrscheinlichkeitsverteilung seine
 Kanten sind. Jeder Pfad des Beam läuft als parallele Fragen ab, sodass zusätzliche Exploration kaum
 Wanduhr-Latenz hinzufügt.
* **Formel:**
 * `path_score = product(edge_probabilities) ** (1 / decisions)`
 * zur Pruning und zum Vergleich von Pfaden verwendet
 * `separation = top_path_score / second_path_score`
 * nützliche Metrik, aber nicht zur Pruning verwendet
 * das Verhältnis vergleicht das geometrische Mittel des führenden Pfades mit dem seines nächsten Rivalen.
 * Nahe `1×` ist mehrdeutig
 * Ein großes Verhältnis bedeutet klare Trennung.
* **Hinweise zu Metriken:**
 * eine andere Metrik wie `min(top_prob/second_top_prob)`, die für
 Pfade optimiert, die an jedem Knoten sehr klare Entscheidungen aufweisen.
 * `exp(mean(log(probs)))` anstelle von `product(edge_probabilities) ** (1 / decisions)` verwenden,
 um Genauigkeitsfehler für sehr tiefe Hierarchien (z. B. >10 Ebenen) zu vermeiden.

## Laden und Visualisieren der Beispiel-Hierarchien

Diese Hilfsprogramme laden die fest verankerten Taxonomie-Quellen herunter, parsen sie zu Direct-Child-Bäumen und rendern jede Suchtraversierung als statisches SVG.
...
## Greedy- und Beam-Search implementieren

Jede Geschwistermenge wird zu einer `Choice`-Frage im nächsten Abschnitt, die auch beide Traversierungsstrategien implementiert und die Wahrscheinlichkeiten beibehält, die die statischen Diagramme benötigen.
...
## Vergleichen Sie die Methoden

Führe beide Strategien an vier beschrifteten Beispielen aus, vergleiche ihre Blätter mit den erwarteten Klassifizierungen und visualisiere die von ihnen erkundeten Routen.
...
Jede Beispiel hat einen bekannten erwarteten Blattknoten. Die Beam-Suche hat 4 von 4 erwarteten Blattknoten getroffen; die Greedy-Suche hat 2 von 4 getroffen. Die Beibehaltung von drei Pfaden hat die erwartete Klassifizierung für CPC-Patente und Shopify-Produkte wiederhergestellt.

| Hierarchie | Erwartetes Blatt | Greedy-Blatt | Beam K=3 Blatt | Greedy korrekt | Beam korrekt |
| -------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------- | -------------- | ------------ |
| CPC-Patente | A01K31/12 Sitzstangen für Geflügel oder Vögel, z. B. Stangen | E99Z99/00 Sonstige Sachverhalte, die in diesem Abschnitt nicht anderweitig behandelt werden | A01K31/12 Sitzstangen für Geflügel oder Vögel, z. B. Stangen | nein | ja |
| Shopify-Produkte | Katzennetz-Fensterbetten & Sitzstangen | Haustier-Sessel | Katzennetz-Fensterbetten & Sitzstangen | nein | ja |
| MeSH-biomedizinische Themen | C06.405.469.432.500 Morbus Crohn | C06.405.469.432.500 Morbus Crohn | C06.405.469.432.500 Morbus Crohn | ja | ja |
| CookSafe-Dateien | retrievers.py | retrievers.py | retrievers.py | ja | ja |

Die Diagramme zeigen, warum sich die Methoden unterscheiden. Orange markiert die gierige Route, grün die siegreiche Beam-Route, lila andere beibehaltene Pfade, und gestrichelte Kanten wurden beschnitten.
