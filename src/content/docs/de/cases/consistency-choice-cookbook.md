---
title: "Selbstkonsistenz: Entscheidungen"
description: "Fügen Sie ein unsicheres Ergebnis zu den Moderationsentscheidungen hinzu und vergleichen Sie die Übereinstimmung der Labels mit dem Anteil der automatischen Maßnahmen."
section: cases
order: 150
...
translatedFrom: en
---
Dieses Kochbuch nimmt einen grenzwertigen Benutzerbeitrag, wendet einen Moderationsraster 15 Mal darauf an
und prüft, ob jede Antwort über die Wiederholungen hinweg stabil bleibt. Jede Prüfung ist ein
`Choice`, sodass jede Antwort ein Label aus einem festen Satz ist. In einer Moderationspipeline
ist dieses Label die Routing-Entscheidung: entfernen oder stehen lassen, eskalieren oder automatisch lösen, an die
Drohung-, Spam- oder allgemeine Warteschlange senden. Wenn das Label von einem Durchlauf zum nächsten wackelt, wird
derselbe Beitrag aus keinem guten Grund an verschiedene Stellen geroutet.

Die Rubrik umfasst 8 `Choice` Fragen, und jeder Lauf ist ein Aufruf, der alle 8 beantwortet. Wir führen
15
Wiederholungen pro Bedingung durch, wobei eine Bedingung ein Modell plus eine Einstellung ist, und zeichnen jedes
zurückgegebene Label auf.

Die Bedingungen:

* Nicht-Reasoning-LLMs `claude-haiku-4-5` und `gpt-5.4-mini`, bei Temperatur `0` und dem
 API-Standardwert.
* Reasoning-LLMs `gpt-5.5` und `claude-opus-4-8`, die keinen Temperatur-Drehregler haben.
* TypeSafe: ein `system_one`-Aufruf über die 8 `Choice`-Fragen hinweg, mit einem neuen `uid`-
 Feld (ein einmaliger, wegwerfbarer Wert) bei jedem Aufruf, passend zum
 Noul-Cookbook-Setup.

Worauf man achten sollte: Ausgewählte Labels können innerhalb einer einzigen Bedingung umschlagen, einschließlich TypeSafe,
und Bedingungen widersprechen sich gegenseitig.

In diesem Lauf wiederholen die LLM-Verteilungseinstellungen ihre Pluralitätslabels 87,5 % bis 100 % der Zeit, im Vergleich zu 90,8 % bei TypeSafe. TypeSafe weist eine geringere mittlere Wahrscheinlichkeitsvariation als fünf der sechs LLM-Verteilungsbedingungen auf; Haiku bei Temperatur 0 variiert weniger. Nahe liegende Wahrscheinlichkeiten ermöglichen dennoch Routing-Änderungen: TypeSafe wechselt bei 2 von 8 Fragen.

Für Anwendungsentscheidungen fordern wir ebenfalls eine maximale Wahrscheinlichkeit von mindestens `0.60`; andernfalls
ist das Ergebnis `uncertain` und wird zur menschlichen Überprüfung weitergeleitet. Die Übereinstimmung von TypeSafe steigt dann auf
99,2 %, mit automatischen Labels bei 74,2 % der Antworten. Wir zeigen die Rohausgaben und wenden den
gleichen Schwellenwert auf LLM-Wahrscheinlichkeitsbedingungen an, wobei Auslassungen und Änderungen sichtbar bleiben.
...
## Der Zustand: ein Benutzerbeitrag an der Grenze, als JSON

Der folgende Beitrag ist darauf ausgelegt, auf der Zaunlinie zu balancieren. Die Sprache ist hitzig und beleidigend, gerichtet teils gegen eine Person und teils gegen das Argument und die Community. Er enthält eine Einladung außerhalb der Plattform (einen Link, der Menschen auf eine andere Site zieht), einen vorherigen Verstoß auf dem Konto und vier Nutzerberichte, und die drohende Formulierung ist nie sauber ausgedrückt.

Hier gibt es keine offensichtliche Einzelantwort, und genau das ist der Punkt: Kleine Wortunterschiede sollten denselben Beitrag nicht willkürlich zwischen den Durchsetzungspfaden verschieben.

Die LLMs erhalten `json.dumps(POST)` im Prompt. TypeSafe erhält das Python-Dict direkt.
...
Jede Frage enthält ein `key`, eine Zeile mit Anweisungen und einen festen Label-Satz. Die Labels innerhalb einer Frage sind sich gegenseitig ausschließend (es gilt genau eines), und jedes trägt eine kurze Beschreibung. TypeSafe gibt ein ausgewähltes `choice` sowie eine `probabilities`-Verteilung über die Labels zurück. Die LLMs werden gebeten, dieselben Label-Sätze zu verwenden, wodurch jede Zeile vergleichbar bleibt.
...
Jeder LLM-Aufruf ist ein Prompt, der `json.dumps(POST)`, alle 8 Fragen und jedes erlaubte Label enthält. Es gibt zwei Antwortformate. Im Verteilungsmodus gibt das Modell ein JSON-Objekt pro Frage mit einer Wahrscheinlichkeit für jedes Label zurück. Im Single-Pick-Modus gibt es ein einzelnes, nacktes Label pro Frage zurück, und unsere Analyse legt die gesamte Wahrscheinlichkeitsmasse auf dieses Label.
...
Jede Abfrage erhält zudem einen frischen `uid`, einen einmaligen, vergänglichen Wert, der sich bei jedem Durchlauf ändert, während der Beitrag und die Bewertungskriterien unverändert bleiben. Er erscheint im LLM-Prompt sowie als zusätzliches Feld im TypeSafe-Status. Diese Konfiguration kann die Empfindlichkeit gegenüber dem irrelevanten Feld nicht von Variationen trennen, die auch bei identischen Anfragen auftreten würden.

Jeder Helfer gibt die Antwort, eine geschätzte Kosten und die Latenz der Hin- und Rückreise zurück.
...
| Modellgruppe | Modell | Verteilung (t=0) | Verteilung (Standard) | Einzelwahl (t=0) |
| ---------------------- | -------------------------------- | :--------------: | :------------------: | :--------------: |
...
* Single-Pick-Bedingungen liefern ein Label pro Frage.
* Temperatur `0` wird häufig für Reproduzierbarkeit empfohlen, daher wird sie mit dem API-Standard verglichen.
...
Eine Zeile entspricht einer vollständigen Rubrikabfrage mit 8 Fragen. `time/call` und `cost/call` mitteln die 15 Abfragen, und die `vs ts_choice`-Spalten teilen durch die TypeSafe-Werte. Die LLMs laufen in einem Pool mit 16 parallelen Instanzen.
...
In dieser Ausführung `typesafe_choice` beträgt die durchschnittliche Hin- und Rücklauf-Latenz 114 ms. Die LLM-Bedingungen reichen von 826 ms bis 13,0 Sekunden pro Aufruf unter den oben genannten Konfigurationen für Parallelität.

## Plot: jede Entscheidung einer Stichprobe als Heatmap
...
* Zellentext: die Anwendungsentscheidung sowie die Wahrscheinlichkeit im oberen Label.
* Zellfarbe: die Position des Labels innerhalb dieser Frage, sodass dieselbe Farbe über die gesamte Zeile hinweg bedeutet, dass die Entscheidung jedes Mal dieselbe ist.
* Grau `uncertain`: die höchste Wahrscheinlichkeit liegt unter `0.60`, daher wird der Fall zur menschlichen Überprüfung weitergeleitet.
* Gestreift `n/a`: die Antwort ließ sich nicht in verwertbare Labels parsen (ein Parsing-Fehler).
* Leere Zeilen sind lediglich Platzhalter.
...
Die klareren Fragen bleiben stabil: `target` liest Person und `severity` liest High durchgängig. Die Grenzfälle verteilen sich auf die Bedingungen: `category`, `primary_risk`, `action`, `review_path` und `link_handling`. Einige Bedingungen wechseln auch innerhalb ihrer eigenen 15 Wiederholungen. Vor der Enthaltung ändert TypeSafe sein Top-Label bei `primary_risk` (Belästigung 11 Mal, Gewalt 4 Mal) und `link_handling` (RmLink 8 Mal, Brigade 7 Mal). Beide Zeilen zeigen nun durchgängig `uncertain`, da ihre höchsten Wahrscheinlichkeiten unter `0.60` liegen.

## Wahrscheinlichungs-Standardabweichung

Dies betrachtet die vollständigen Wahrscheinlichkeitsvektoren, nicht nur das ausgewählte Label. Für jede Bedingung sammeln wir alle 15 Verteilungen für jede Frage, berechnen die Standardabweichung der Wahrscheinlichkeit jedes Labels über die Wiederholungen hinweg (wie stark sie von Lauf zu Lauf schwankt) und mitteln dann diese Standardabweichungen über alle Labels und Fragen. Wir berichten auch über die größte einzelne Standardabweichung eines Labels und zählen Parsing-Fehler separat.

Der Vergleichstabelle werden alle LLM-Bedingungen, die Wahrscheinlichkeitsausgaben erzeugen, gegen TypeSafe gegenübergestellt. Die Zeilen mit der einzelnen Auswahl sind ausgelassen, da sie harte Labels statt Wahrscheinlichkeitsverteilungen ausgeben.
...
In diesem Durchlauf weist TypeSafe eine mittlere Standardabweichung der Wahrscheinlichkeit von `0.0098` und eine maximale Standardabweichung für einzelne Labels von `0.0515` auf. Haiku bei der Temperatur 0 hat eine niedrigere mittlere Standardabweichung von `0.0012`. Die anderen fünf LLM-Wahrscheinlichkeitsbedingungen reichen von `0.0245` bis `0.0543`, was etwa `2.5x` bis `5.6x` des TypeSafe-Mittels entspricht. Kleine Änderungen können immer noch das Top-Label wechseln, wenn zwei Labels nah beieinander liegen.

## Plot: Entscheidungsvereinbarung mit ungewissem Ausgang

Gib `uncertain` zurück, wenn die höchste Wahrscheinlichkeit unter `0.60` liegt. Zähle für jede bedingungsbasierte Wahrscheinlichkeitsausgabe und jede Frage die häufigste Anwendungsentscheidung, einschließlich `uncertain`, und teile sie durch alle 15 Ziehungen. Parsing-Fehler werden gegen die Übereinstimmung gewertet. Jeder Balken mittelt die Punktzahl über alle 8 Fragen, wobei die höchste Übereinstimmung zuerst kommt.

Single-Pick-LLM-Bedingungen sind ausgeschlossen, da sie keine Unsicherheitsschätzung liefern.
...
Unter derselben `0.60`-Regel erzielte Haiku bei Temperatur 0 eine Trefferquote von 100 %. TypeSafe erreichte 99,2 %, und die anderen LLM-Bedingungen lagen zwischen 84,2 % und 94,2 %. TypeSafe gab bei 25,8 % der Antworten `uncertain` zurück und handelte automatisch in den anderen 74,2 % der Fälle; Haiku bei Temperatur 0 verzichtete niemals auf eine Antwort. Diese Prozentsätze messen ausschließlich die Wiederholbarkeit. Die folgende Tabelle stellt die rohe Übereinstimmung und die Verzichtsraten neben der Policy-Übereinstimmung in diesem Diagramm dar.

## Lassen Sie unsichere Wahrscheinlichkeiten zu einer unsicheren Entscheidung führen

Eine kleine Änderung der Wahrscheinlichkeit kann zwei nahe beieinander liegende Labels austauschen. Die Anwendung muss nicht auf den Gewinner reagieren: Gib `uncertain` zurück, wenn die höchste Wahrscheinlichkeit unter `0.60` liegt, und leite diesen Fall an einen Menschen weiter. Wähle bei genau `0.60` das Label mit der höchsten Wahrscheinlichkeit. Dabei werden die zurückgegebenen Wahrscheinlichkeiten verwendet, nicht das separate Feld `confidence` der API, und es werden keine zusätzlichen Modellaufrufe benötigt.

Die Schwelle ist eine illustrische Anwendungsrichtlinie, keine kalibrierte Garantie oder eine
Schwelle, die gewählt wurde, um die Übereinstimmung dieses Durchlaufs zu maximieren. Wählen Sie Produktionsschwellen mit
beschrifteten Beispielen und den Kosten für fehlerhafte Aktionen und menschliche Überprüfung.

Wir wenden dieselbe Regel auf jede bedingte Wahrscheinlichkeitsausgabe an. Einzelne LLM-Antworten
haben keine Wahrscheinlichkeitsschätzung; ihre synthetischen One-Hot-Vektoren können Unsicherheit
nicht messen, daher werden sie aus der Einigungsübersicht und -tabelle ausgeschlossen.
...
`policy agree` wertet `uncertain` als Entscheidung; Parsing-Fehler zählen gegen die Vereinbarung.
`automatic` ist der Anteil aller Antworten, die ein Label auswählen. `conflicts` zählt Fragen
mit mehr als einem konkreten Label über die Wiederholungen hinweg, unter Ignorierung
von Enthaltungen. Diese Maßnahmen beschreiben Wiederholbarkeit und wie oft die Anwendung
handelt, nicht ob ihre Handlungen richtig sind.

Die Übereinstimmung von TypeSafe stieg von 90,8 % auf 99,2 %. Von den Antworten waren 25,8 % unsicher und
74,2 % automatisch. `primary_risk` und `link_handling` blieben bei jeder Wiederholung unsicher;
`category` wechselte zwischen Gewalt und `uncertain`, überschritt bei einigen Wiederholungen die Aktionsschwelle, bei anderen nicht. Keine Frage führte zu zwei verschiedenen konkreten TypeSafe-Labels.
Dies alles zeigt keine Genauigkeit oder Überlegenheit: Haiku bei Temperatur 0 hatte hier eine Übereinstimmung von 100 %, ohne Enthaltungen.
...
Diese Richtlinie macht das Modell nicht deterministisch. Das Zurückhalten kann konkurrierende
Labels durch dasselbe Ergebnis der menschlichen Überprüfung ersetzen, aber eine Wahrscheinlichkeit nahe `0.60` kann immer noch
zwischen einem konkreten Label und `uncertain` wechseln. Die Wahrscheinlichkeitsstatistiken und die Spalte `raw
agree` der Tabelle melden weiterhin die ursprünglichen Modellausgaben.

## Öffnen Sie es im TypeSafe-Playground

Der folgende Link öffnet denselben Beitrag und dieselbe Rubrik im Playground: ein Beitrag, dieselben 8
`Choice`s und TypeSafe `jev-latest`.
