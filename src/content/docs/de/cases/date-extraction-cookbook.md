...
description: "Extrahiert absolute und relative Daten, indem es TypeSafe nach den in einem Dokument benannten Teilen fragt und diese anschließend im Code durch auflösen und validieren lässt, wobei eine vertrauensbasierte Überprüfung erfolgt."
section: cases
order: 170
...
translatedFrom: en
---
*Lies die Teile eines Datums aus dem Text mit TypeSafe ab und löse sie dann in Code zu einem `date` auf.*

Die Funktion, die Sie hier erstellen, `extract_date(document, role)`, nimmt ein Dokument und eine Phrase, die das gewünschte Datum benennt, wie zum Beispiel „die Frist zur Rückgabe des Formulars“, und gibt ein `date` mit einer Konfidenz zurück. Sie markiert eine Lesung mit niedriger Konfidenz sowie eine, deren Teile überhaupt kein Datum ergeben, einschließlich eines Datums, das das Dokument niemals angibt. Das Datum kann ausgeschrieben sein („14. August 2027“) oder relativ zu heute geschrieben sein („morgen“, „nächsten Donnerstag“).

TypeSafe beantwortet `Choice` Fragen zum Datum in einem Aufruf: welche Art von Datum es ist,
und
welchen Monat, Tag, Jahr oder Wochentag der Text nennt. Code wandelt diese Antworten in ein `date` um.
Das Modell liest, was der Text sagt, und führt niemals Kalenderberechnungen durch.

Die folgenden Zellen führen diese Funktion über vier kurze Dokumente aus, geben jedes Datum zusammen mit seiner Konfidenz aus und teilen die Ergebnisse in die Kategorien auf, die Code akzeptiert, und die, die eine Person prüfen sollte.
...
*TypeSafe liest, wie das Datum geschrieben ist und welche Teile der Text nennt. Code wandelt diese
Antworten in ein `date` um, zählt ab heute, wenn das Datum relativ ist, und akzeptiert es entweder
oder sendet es zur Prüfung.*
...
Sieben `Choice` Fragen werden in einem Aufruf gesendet. `mode` legt fest, wie das Datum geschrieben wird:
`absolute`
für ein Datum, das einen Monat benennt, `relative` für eines, das relativ zu heute geschrieben ist, und `none`
wenn das Dokument das Datum überhaupt nicht angibt.

Die anderen sechs lesen die Teile. Ein absolutes Datum benötigt `month`, `day` und `year`. Ein relatives benötigt `day_anchor`: heute, morgen, übermorgen oder einen benannten Wochentag. Wenn es einen Wochentag nennt, sagen `weekday` und `week_offset`, welcher und welche Woche. Der Code liest nur die Teile, die `mode` anfordert.

`year` listet eine Option pro Jahr von 1900 bis 2050 sowie zwei Ausweichmöglichkeiten. `none` bedeutet, dass der Text kein Jahr angibt und der Code eines einfügt. `out_of_range` bedeutet, dass der Text ein Jahr angibt, das nicht in der Liste enthalten ist, und der Code dies anstelle eines Raten meldet. Wenn eine so lange Liste störend wirkt, ziehen Sie zunächst die jahresartigen Zahlen aus dem Text heraus und bieten Sie dem Modell nur diese an.
...
`read_parts` trifft die Entscheidung. `assemble` wandelt die Antworten in ein `date` um: Es ergänzt das Jahr, wenn der Text keines angibt, und ermittelt, auf welchen Tag ein genannter Wochentag verweist. Beide zählen von `TODAY` aus, das festgelegt ist, damit relative Datumsangaben bei jeder Ausführung gleich herauskommen. `assemble` meldet zudem das niedrigste Konfidenzniveau unter den verwendeten Teilen, sodass eine schwache Antwort in einem Teil das gesamte Datum zur Überprüfung markieren kann.

„next Thursday“ kann zwei verschiedene Tage bedeuten, daher entscheidet der Code, welcher gemeint ist. Ein Wochentag ohne Zusatz bedeutet den nächsten, der auf oder nach heute folgt. `next` bedeutet die folgende Kalenderwoche, und `current` bedeutet diese Woche.
...
Sechs Fragen in vier kurzen Dokumenten: zwei Daten aus einem Vertrag, der seine Laufzeit angibt,
eine Frist für ein Formular ohne Jahresangabe, eine Umfrage, die „heute“ endet, eine Bewertung für
„nächsten Donnerstag“ und ein Datum, das das Formular nie erwähnt. Alle lösen sich gegenüber `TODAY` =
2026-07-30, einem Donnerstag, auf.
...
Der Vertrag gibt beide Jahre an, daher wurden diese aus dem Text entfernt. Das Formular gibt kein Jahr an,
also hat der Code 2026 eingetragen: Er nimmt das aktuelle Jahr und wechselt erst zum nächsten, wenn das
Datum bereits mehr als einen Monat zurückliegt. „Heute“ und „nächsten Donnerstag“ durchliefen die gleiche
Funktion wie die ausgeschriebenen Datumsangaben.

Der Kickoff-Anruf ist derjenige, den das Formular nie erwähnt. In diesem Formular steht ein Datum, nur nicht in diesem hier, und die Notiz `absolute date incomplete` bedeutet, dass `mode` mit `absolute` zurückkam, ohne einen passenden Monat. Das Datum kam leer zurück, die Konfidenz beträgt 0,46 und die Zeile wurde für eine Person markiert.

## Vertrauen, um darauf zu routen

Jede Antwort wird mit einem kalibrierten Konfidenzwert zurückgegeben, und das Konfidenzniveau eines Datums ist der niedrigste Wert unter den Teilen, die in es eingeflossen sind. Ein Datum unter `REVIEW_BELOW` = 0,60 wird einer Person zugeleitet, und dasselbe gilt für ein Datum, das der Code nicht zusammenstellen konnte. Der Rest wird direkt durchgelassen.
...
Der untenstehende Link enthält die „nächsten Donnerstag“-Nachricht und dieselben Fragen, die der Code sendet.
Öffnen Sie ihn, um die Antworten und ihre Konfidenzwerte einzusehen sowie die Formulierung zu ändern, ohne Code zu schreiben.
