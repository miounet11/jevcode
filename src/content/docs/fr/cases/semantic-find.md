---
title: "Recherche ligne par ligne"
description: "Mettre en place une recherche sémantique pour les conditions d'utilisation de GitHub. En une seule requête, évaluer 218 identifiants de lignes par rapport à une requête en langage courant à l'aide d'une question à choix, et utiliser une question Noul pour vérifier si le document contient une réponse."
section: cases
order: 260
...
translatedFrom: en
---
Vous avez les Conditions d’utilisation de GitHub et une question formulée en langage courant à leur sujet. Vous avez besoin des lignes qui répondent à la question et d’un moyen de détecter lorsque le document ne contient aucune réponse. Les requêtes incluses classent en premier les lignes contenant des réponses directes. Les seuils `exists` classent les cas restants comme manquants ou partiels. Vous obtenez ainsi `find()`, qui renvoie la probabilité `exists` et un score de pertinence par ligne.

<img src="/img/cases/semantic-find-semantic-find-hero.gif"
 alt="Une requête analyse un document et révèle une réponse attachée à la ligne correspondante"
 width="1120"
 height="520"
 data-path="cookbooks/semantic_find/semantic-find-hero.gif"
/>

Le backend de recherche se compose de trois parties :

1. Attribuer un ID à chaque ligne afin que TypeSafe puisse y faire référence.
2. Utiliser une question `Choice` pour classer ces IDs de ligne selon leur capacité à répondre à la requête. Les probabilités d'une question de type Choice s'additionnent toujours à 1, de sorte qu'une ligne est classée première même si aucune ne répond à la requête.
3. Dans la même requête, utiliser une question `Noul` pour vérifier si le document contient une réponse.
...
`JsonCache` rejoue les réponses API incluses, de sorte que les étapes ci-dessous s’exécutent sans clé API
ni frais. Pour rendre les requêtes réelles, définissez `TYPESAFE_API_KEY` et supprimez
`json_cache.json`.
...
Maintenant, préfixez chaque ligne par un court identifiant et rejoignez les lignes pour former un seul document. Le modèle utilise ces identifiants pour pointer vers sa réponse.
...
Une `Choice` question renvoie une probabilité pour chaque option. Utilisez les IDs de ligne comme options,
et « choisir une option » devient « pointer une ligne. »
...
> **Note** — Une question `Choice` accepte jusqu’à 255 options, donc cette recette recherche des documents d’au plus 255 lignes en une seule requête. Au-delà, effectuez la recherche en deux passes : une question Choice sélectionne une fenêtre de lignes, puis une seconde classe les lignes à l’intérieur de celle-ci.


## Étape 3 : vérifier si une réponse existe

Les probabilités de choix s’additionnent toujours à 1, de sorte qu’une ligne arrive en première position même lorsque le document ne répond pas à la question. Le seul classement ne permet pas de distinguer une réponse réelle d’une ligne non pertinente la plus proche.
...
Contrairement aux probabilités de Choice, la probabilité de Noul ne dépend pas des autres options,
elle peut donc chuter près de zéro lorsque le document ne contient pas de réponse.

## Étape 4 : envoyer les deux questions dans une seule requête

La méthode `system_one` répond aux deux questions en un seul passage. L’état est envoyé une seule fois, de sorte que l’ajout de la vérification d’existence ne nécessite qu’une petite quantité de sortie supplémentaire.

<img src="/img/cases/semantic-find-recipe.png"
 alt="Un document étiqueté et une question utilisateur entrent dans une seule requête TypeSafe. Une question Choice évalue
...
Deux morceaux de code local terminent le travail : `verdict()` transforme la probabilité brute `exists` en trois états, avec un état intermédiaire pour les réponses partielles, et `show()` rend `relevance` sous forme de graphique en barres afin que le classement soit lisible dans un terminal.
...
* **Autorisation parentale :** La règle relative à l’âge arrive en tête, mais elle ne précise pas si l’autorisation parentale modifie cette règle. Le résultat est **partiellement abordé**.

Le classement vous indique où chercher ; le score `exists` vous dit si le résultat répond à la question.

## Essayez-le sur votre propre document

[Ouvrez le contrat tagué dans le playground TypeSafe][playground] pour modifier les questions
par rapport au même texte. Pour rechercher les vôtres, remplacez l'URL dans ⦇0⦇ ; chaque ligne
du script fonctionne à partir de ⦇1⦇.
