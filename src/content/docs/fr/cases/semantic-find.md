...
description: "Construisez une recherche sémantique pour les Conditions d'utilisation de GitHub. En une seule requête, évaluez 218 identifiants de lignes par rapport à une requête en langage courant avec une question Choice, et utilisez une question Noul pour vérifier si le document contient une réponse."
section: cases
order: 260
...
Une question `Choice` renvoie une probabilité pour chaque option. Utilisez les IDs de ligne comme options,
...
Les descriptions des options sont `None` car le document contient déjà le texte pour chaque
ID. La requête va dans `instructions` ; l'état reste inchangé entre les recherches.
...
Les probabilités de choix s'additionnent toujours à 1, donc certaines lignes arrivent en premier rang même lorsque le document
ne répond pas à la question. Le seul classement ne permet pas de distinguer une réponse réelle de la
ligne non pertinente la plus proche.
...
elle peut donc être proche de zéro lorsque le document ne contient pas de réponse.
...
<img src="/img/cases/semantic-find-recipe.png"
 alt="Un document tagué et une question utilisateur entrent dans une seule requête TypeSafe. Une question Choice évalue
...
Le classement vous indique où chercher ; le score `exists` vous indique si le résultat
répond à la question.

## Essayez-le sur votre propre document

[Ouvrir le contrat balisé dans le playground TypeSafe][playground] pour modifier les questions
par rapport au même texte. Pour rechercher le vôtre, remplacez l'URL dans `fetch_document()` ; chaque autre
ligne du script fonctionne avec `LINES`.
