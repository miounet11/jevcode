---
title: "行ごとの検索"
description: "GitHubの利用規約に対するセマンティック検索を構築する。1つのリクエストで、Choice質問を用いて218の行IDを平易な言語のクエリに対してスコアリングし、Noul質問を用いて文書に回答が含まれているかを確認する。"
section: cases
order: 260
...
translatedFrom: en
---
GitHubの利用規約と、それに関する平易な質問があります。質問に答える行と、文書に回答がないことを検出する方法が必要です。含まれるクエリは、直接的な回答を含む行を優先してランク付けします。`exists`の閾値は、残りのケースを欠落または部分的な回答として分類します。最終的に`find()`が得られ、これは各行に対して`exists`の確率と1つの関連性スコアを返します。

<img src="/img/cases/semantic-find-semantic-find-hero.gif"
 alt="クエリが文書をスキャンし、一致する行に付随する回答を表示する"
 width="1120"
 height="520"
...
1. TypeSafeが各行を指し示せるよう、各行にIDを付与する。
2. `Choice`の質問を用いて、それらの行IDがクエリにどれだけ適切に回答しているかによって順位付けする。Choice
 質問の確率は常に1に合計されるため、クエリに回答する行がなくても、ある行が第1位となる。
3. 同じリクエスト内で、`Noul`の質問を用いて、ドキュメントに回答が含まれているかどうかを確認する。
...
`JsonCache` 含まれるAPIレスポンスを再生するため、以下の手順はAPIキーや課金なしで実行されます
リクエストを実際に実行するには、`TYPESAFE_API_KEY`を設定し、`json_cache.json`を削除してください。
...
テスト文書は GitHub の利用規約であり、218条項に分割されているため、検索結果はすべて引用可能な1行を指し示します。

`semantic_search.py`に追加：
...
各ラインの先頭に短い ID を付加し、ラインを結合して 1 つのドキュメントに戻します。モデルはこれらの ID を使用して自身の回答を参照します。
...
`Choice`の質問は、各選択肢に対して確率を返します。行IDを選択肢として使用し、「選択肢を選ぶ」は「行を指す」になります。
...
オプションの説明は`None`です。なぜなら、各IDに対応するテキストがすでに文書に含まれているからです。クエリは⦇1⦇に入力されます。検索間、状態は変更されません。


> **注** — `Choice` 質問は最大255個のオプションを受け付けるため、このレシピは1回のリクエストで最大255行のドキュメントを検索します。それを超える場合は、2段階で検索してください。最初の Choice 質問で行のウィンドウを選択し、2番目でその中にある行をランク付けします。


## ステップ3：回答が存在するか確認する

Choiceの確率は常に合計で1になるため、文書が質問に答えていない場合でも、ある行が最初にランク付けされます。ランク付けだけでは、実際の回答と最も近い無関係な行を区別できません。

2つ目の質問を、同じリクエストの中で尋ねてください：
...
Choiceの確率とは異なり、Noulの確率は他の選択肢に依存しないため、
ドキュメントに回答がない場合、ほぼゼロに近づくことがある。

## ステップ4: 両方の質問を1つのリクエストで送信

`system_one`メソッドは、両方の質問を一度の処理で回答します。状態は1回だけ送信されるため、存在チェックの追加にはわずかな追加出力のみが必要です。

<img src="/img/cases/semantic-find-recipe.png"
 alt="タグ付けされたドキュメントとユーザーの質問が1つのTypeSafeリクエストに入力される。Choiceクエリは各行をスコアリングし、Noulクエリは回答が存在するかを確認する。ローカルコードはその後、行をランク付けし、ドキュメントの判断を適用する。"
 width="2170"
 height="458"
...
ローカルコード2つで完了です：`verdict()`は生の`exists`確率を、部分的な回答用の中間状態を含む3つの状態に変換し、`show()`は`relevance`を棒グラフとしてレンダリングして、ターミナルでランキングが読み取り可能になります。
...
これらの閾値は以下の例を区切りますが、本番環境で使用する前に、ご自身のドキュメントに合わせて調整してください。
