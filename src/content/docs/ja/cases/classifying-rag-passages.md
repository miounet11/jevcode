---
title: "RAGパッセージの分類"
description: "取得した各段落に対してTypeSafeリクエストでスコア付けを行い、その後コード内で回答モデルに渡すものを決定する。例えば、質問と矛盾するものは保持してフラグを立て、隠れた指示やプロンプトインジェクションを含むものは除外する。"
section: cases
order: 140
...
translatedFrom: en
---
RAGパイプラインの検索ステップでは、クエリとの語句の類似度に基づいてパッセージをランク付けし、上位のパッセージを言語モデルに渡します。これにはノイズの多いまたは無関係なパッセージが含まれる場合があり、さらに悪いケースでは、矛盾する事実、プロンプトインジェクション、またはモデルの指示が、回答生成を支援するための形式的な証拠と混在することがあります。

検索と生成の間に、取得した各パッセージを分類する第二段階を追加する。各パッセージについて、TypeSafe に対して、クエリ–パッセージのペアに関する複数の質問を含む1つのリクエストを送信する：それは関連性があるか、回答に使用可能な情報を示しているか、クエリが前提としているものと矛盾しているか、モデルに指示しようとしているか。これらの質問への回答に基づき、各パッセージの処理が単純な分岐ロジックで決定される：プロンプトに証拠として追加するか、矛盾情報として追加するか、または除外するか。証拠と矛盾情報は別々のブロックとして渡されるため、ジェネレーターは適切に対応できる。

パイプラインを実行するには、実在する認証ドキュメント（ページ間で見分けがつかない内容が大量に含まれる）と、プロンプトインジェクションを含む仕掛けられた文章に対して、いくつかの厄介な質問を投げかけます。2つの質問には誤った前提が含まれており、回答を生成するモデルに渡される前にこれらはフラグ付けされます。

パイプラインは、セクションが構築する順序で構成される：81件からなるコーパス、各クエリにつき上位12件のパスセージを保持するコサイン類似度検索、それらのパスセージそれぞれに対して`Noul`でTypeSafeに送信される4つの質問、`route()`で各質問にラベルを付けるしきい値、個別の証拠ブロックと競合ブロックから組み立てられたプロンプト、そしてそれに基づいて`claude-sonnet-5`が出力する回答。
...
| ノード | 説明 | グループ |
| :--- | :--- | :--- |
| `CALL` | 取得した各パスagesごとに1リクエスト | 取得した各パスagesごとに1リクエスト |
| `N` | Nouls: / 関連性あり？/ 利用可能な証拠を提示している？/ クエリの前提と矛盾している？/ モデルに指示を出している？ | 取得した各パスagesごとに1リクエスト |
| `GEN` | 1回のLLM呼び出し | 1回のLLM呼び出し |
| `INC` | 承認された証拠 | 1回のLLM呼び出し |
...
`TYPESAFE_API_KEY`、`ANTHROPIC_API_KEY`、`OPENAI_API_KEY`を設定する。各取得した断片に対してTypeSafeでスコアリングを行い、検索ステップのためにOpenAIを用いてコーパスを埋め込み、スコアリングを通過した内容に基づいてClaudeが最終回答を生成する。

この3つはいずれも、このページを再現するためにキーを必要としない。`json_cache.json`はcookbook同梱で、記録された呼び出しをすべて再生するため、再レンダリングにコストはかからない。ファイルを削除すると、代わりにパイプラインをライブで実行できる。ここでの数値は、2026-08-27時点の`jev-1.12`および`claude-sonnet-5`から得られたものである。
...
各パスには⦇0⦇、⦇1⦇、⦇2⦇、⦇3⦇が含まれ、すべてのリクエストはこれら4つを送信する。ニアミスがセットを埋める。ローテーション、有効期限、セッション、署名キーはそれぞれ独自のページを持ち、それらのページは似たような読み心地である。リフレッシュトークンのローテーションとJWT署名キーのローテーションは、ほぼ同じ言葉で記述されている異なるものである。

私たちは最後のものを自前で書き、`forum-injection`にマークしました。`community_forum`：これは最後の段落を除き通常のフォーラムの回答として読めますが、その最後の段落はモデルに向けた指示です。

また、6つのクエリのうち2つは、ドキュメントが矛盾する前提を述べるように記述したため、インジェクションおよび競合の両ルートに検出対象が存在するようになっている。
...
埋め込みの余弦類似度に基づいて文章をランク付けし、`text-embedding-3-small`の256次元の埋め込みを用いて、各クエリに対して最も良い`TOP_K = 12`を保持します。短いベクトルは shipped キャッシュを小さく保ち、埋め込み呼び出しは他のすべてのものと一緒にキャッシュされるため、ベクトルは`json_cache.json`内で移動します。
...
前提を否定する段落、`sessions-01`は、0.509で7位です。12のスコアすべてが0.584から0.455の間にあり、この幅は狭すぎて、クエリを修正する段落と回答を乗っ取ろうとする段落を区別するには不十分です。

## 各段落について4つの質問を投げかける

クエリと1つのパッセージを状態に一緒に配置し、各質問がパッセージ単独ではなくペアに関するものになるようにする。形状：
...
4つの`Noul`の質問と、それぞれの回答が導くもの：

* `is_relevant`: 関連性の閾値。
* `contains_answer_evidence`: 含めるか、除外するか。
* `contradicts_query_premise`: 衝突ブロックへ昇格させる。
* `contains_prompt_injection`: 完全に除外する。

4つのいずれも、その記述を含めるかどうかを問うてはいない。その判断は以下のコードにあり、そこを変更するとは、質問を言い換えるのではなく、数値を編集することを意味する。
...
すべての回答は確率として返され、それらの4つを1つの意思決定に変換する方法は数多く存在します。ここでは単純な比較の連鎖が機能しました。4つの確率を固定された順序でしきい値と比較し、最初に一致したところで停止します。その一致がパッセージにラベルを付け、そのラベルがパッセージの処理内容、つまりプロンプト内の証拠、プロンプト内の矛盾、または破棄を決定します。
...
インジェクションは、証拠に基づく判断ではなくセキュリティ上の判断であるため、最初に実施されます。矛盾テストは証拠テストよりも前に実施されます。なぜなら、クエリの前提を否定する記述は通常、有用な情報も含んでいるためです。もし逆の順序でテストすると、その記述は「競合」ブロックではなく「承認」ブロックに分類されてしまうからです。


> **注** — このコーパスのためにこれらの4つの数値を選びました。デフォルトではなく、出発点として扱ってください。1つを変更するのは容易です：`THRESHOLDS`は4つすべてを保持し、`route()`は保存された回答のみを読み取るため、すべてのパッセージをルーティングし直すのにAPI呼び出しはかかりません。
...
前提矛盾質問は `sessions-01` で 0.92 のスコアとなり、衝突ブロックに送られる。関連性は 0.49、回答証拠は 0.51 であり、この2つだけでも却下されるところであった。

`forum-injection`の類似度順位が最優先され、その関連性は0.71の閾値を満たしています。
その低下要因となっているのは、0.99というインジェクションスコアです。

プロンプトには証拠として何も届かないが、これは誤った前提に基づいて構築された質問には正しい。以下は、ドキュメントが回答するクエリに対する同じテーブルである。
...
4つのパスセージがここでの証拠ブロックに到達し、以下の回答はそれら4つすべてを引用している。
行は検索順に印刷され、それによってシャッフルの状況が示される：ランク2、3、4はすべて
*署名キーの寿命* と読まれ、これはほぼクエリ自身の言葉で言えば不適切な寿命であり、
関連性スコアはすべて0.08以下である。選ばれた4つのうち3つは、それぞれ8位、9位、11位にいた。`forum-injection`は0.99で再び除外されている。

インジェクション質問はフィルタであり、唯一のフィルタである。スコアが閾値を下回った文書でもプロンプトに到達するため、ジェネレータープロンプトはスコアに関係なく、すべての文書を信頼できないテキストとして扱う必要がある。ここにはセキュリティ境界は存在しない。

1パスあたり1リクエストなので、コストは`k`に比例して変動します。複数のパスを1つのリクエストにまとめることはありません。各質問は1組のペアに関するものだからです。

## 受け入れられた証拠からプロンプトを構築する

TypeSafeはパスにスコアを付け、ルーティングはそれらをラベル付けする。LLMはまだ回答を書く、
ここ`claude-sonnet-5`で。受け入れられた証拠と矛盾する証拠は別々のブロックに保持する。

2つのブロックにより、回答が反論できる。これらを1つにマージすると、ジェネレーターはクエリに答える記述と、その前提を否定する記述を見分ける手段を持たなくなる。
...
最初の回答は、誤った前提に基づく質問「リフレッシュトークンは30日後に期限切れになる——どうすればその期間を延ばせるのか？」に対するものであり、2番目の回答は、ドキュメントが回答している通常の質問に対するものです。この質問には12の取得されたパッセージが含まれており、その中には`forum-injection`とその注入された指示が含まれていました。
...
最初の回答は、空の承認ブロックと1つの矛盾する記述を持って到着した。それは「承認された証拠が不十分である」として始まり、矛盾点を指摘し、30日の設定をでっち上げるのではなく、`sessions-01`においてリフレッシュトークンが期限切れにならないという引用を示している。
...
各バーには、1つのクエリに対して取得された12の断片が格納されており、合計72個です。各バーの少なくとも3分の2は除外されます。誤った前提に基づく2つのクエリのみが競合へルーティングされ、2つのクエリは全く受け入れません。すなわち、30日間の有効期限に関するものと、*refreshトークンはどのようにローテーションされるのか？* です

## プレイグラウンドで開く

以下のリンクを開いて、1回の呼び出しをライブで再実行してください：衝突ブロックにルーティングされたパスジに対する最初のクエリ、および4つの質問。
