---
title: "事前解析値抽出"
description: "正規表現で候補となるメールアドレス、電話番号、金額を検出し、その後 TypeSafe が要求されたスパンを選択することで、コードが原文のままの値を正規化できるようにする。"
section: cases
order: 230
...
ここで`find`と`pick`のペアは、あなたのドキュメントを指し示すことができるものであり、3つの実例がその使用法を示しています：差出人が領収書を送信したい住所、`+14155550177`としての電話番号、および請求としてフラグ付けされた`1315.50 USD`の請求書合計額。

TypeSafeはあなたが渡したオプションの中から1つを選ぶので、候補をまず見つける必要があります。正規表現でそれらを見つけ、TypeSafeが1つを選び、コードがその選択をコピーする、という3つのステップです：

1. 正規表現でテキスト内の候補値を検出します。過検出されるように調整してください。
2. TypeSafe は、質問が求める候補値を選択し、コードが下流で必要とする属性（通貨、国、金額がクレジットか請求か）を読み取ります。
3. コードは選択された値をコピーし、正規化します。

TypeSafeは正規表現が見つけたスパンの中からのみ選択するため、返される値はそれらのスパンのいずれかで、変更せずにコピーされたものです。値を凭空生成したり、桁を並べ替えたりすることはできません。
...
*正規表現でドキュメント内の候補値を検出し、TypeSafe が1つを選択し、下流のコードがそれを正規化して処理する。*
...
`find`は、過剰検出に調整された正規表現を実行し、重複を除去します。`pick`は、選択肢が`find`が返すスパンである`Choice`質問であり、その回答はそれらのスパンを正確にコピーしたもの、または該当する候補がない場合は`none`になります。`classify`は、通貨と国に対して使用される、固定されたラベルセット上の`Choice`質問です。
...
## メール: 役割に応じて適切なアドレスを選択

ヘッダーには4つの住所がある。本文では、`To:`請求先エイリアスではなく個人宛の住所に領収書を送るよう求められているため、回答は本文を読む必要がある。ここでは2つの質問がある：領収書はどの住所に送るか、そしてメッセージはどの住所から送られてきたか。
...
`receipt`は`Reply-To:`行にある個人用Gmailアドレスであり、本文で求められているものです。`sender`は`From`行にあるアドレスです。両方とも正規表現のマッチ結果のコピーであり、コード内で小文字に変換されています。

## 電話：モバイルを選択し、E.164 に正規化する

国コードを伴わない3つの数値。TypeSafeが携帯電話を特定し、テキストから国を読み取ります；`phonenumbers`はこれらの2つの回答をE.164形式に統合します。E.164は`+`と国コードで始まる国際形式です。
...
桁の数字だけでは、どの番号が携帯電話用で、どの国に属するのかが分かりません。それを判断するのは、周囲の言葉です。TypeSafe はそれらの言葉を読み取り、`phonenumbers` で選ばれた番号を `+14155550177` の形式で整形します。

## お金：金額を選択し、通貨を分類し、クレジットと請求を区別する

4つの金額が記載された請求書。TypeSafeは未払総額とクレジット額を選択し、通貨を読み取り、選択された各金額を請求またはクレジットとしてフラグ付けします。コードは選択された各文字列をコピーし、それを `Decimal` に解析します。
...
合計支払額は1,315.50米ドル、クレジットは50.00米ドルです。クレジットまたは請求
`Noul`は合計に対して0.01、クレジットに対して0.99に回答するため、コードは解析する各`Decimal`の符号を把握します。

> `to_decimal`は、カンマが千の位を区切り、ドットが小数点であると仮定します。これは`$1,315.50`に当てはまりますが、`€1.315,50`ではその逆です。文書がどの慣習を採用しているかを問う`Noul`質問を行い、コード内で分岐してください。

## TypeSafe プレイグラウンドで開く

ブラウザでメールスレッドを開く共有リンクで、領収書の質問と、正規表現がオプションの中から見つけた4つのアドレスが含まれています。
...
* `Choice`の質問では、オプションは最大255個までです。それ以上の候補がある場合は、2段階で絞り込みます：まずセクションを選び、その中の範囲を特定します。
* 候補の特定が作業の中心です。メールアドレス、電話番号、金額には正規表現で対応できますが、名前には対応できないため、候補は既存のリストか、名前を抽出するエンティティ認識モデルやLLMから得る必要があります。その後、TypeSafeが質問で求められているものを選択します。