---
title: "スキル提案"
description: "182のNous Research Hermesカタログから、エージェントのターンに対して最大1つのスキルを選択する。TypeSafeリクエストはまずすべてのスキルをランキングし、そのターンにスキルがそもそも必要かどうかを判断し、2番目のリクエストは上位3つを正しく読み取り、それらすべてを拒否できる場合もある。勝者の名前はエージェントのシ"
section: cases
order: 270
...
translatedFrom: en
---
*エージェントは、すべてのスキルを切り捨ててシステムメッセージに読み込むことでスキルを選択しますが、これによりコストが増加し、スキルの選択パフォーマンスが低下し、セッションの残りの部分でコンテキストの劣化（コンテキスト・ロット）を引き起こします。私たちは、ターンごとに2つのTypeSafeリクエストを使用することでこれを解決します。1つはスキルのランク付け用、もう1つは選択の検証用であり、これにより誤ったスキルの読み込みを半分以下に削減します。*

大規模なスキル roster を持つエージェントは、ほとんど情報がない状態で選択を行う。この roster はインデックスとしてエージェントに渡される：スキルごとに1行ずつ記述され、説明文は切り捨てられており、完全なテキストが会話を圧迫しないようになっている。ここで使用されているエージェントハーネスであるHermesは、デフォルトで60文字に切り詰める。例えば、その幅では *`.pptx`* ファイルを *編集する* スキルと、それらを *作成する* スキルはほぼ同じように読める。ピッチデッキを依頼すると、エージェントは間違ったものをロードする可能性がある。どのスキルも完全に適合しないターンであっても、名前の一覧は推測を誘うため、エージェントはそれでもなお1つをロードすることがある。

このクックブックは説明をそのままにし、段階的な開示を採用して、
182のスキルを低コストですべて読み込んだ後、そのうち3つを詳細に読みます。決定の前に、
どのスキルをロードするか、あるいはロードしないかの判断の前に、2つのTypeSafe
リクエストが行われます。1つ目は、ロースターのすべてのスキルを
ユーザーのターンに対してランク付けし、ターンにスキルが必要かどうかを答えます。2つ目は、各スキルの完全な説明と
その指示の冒頭を伴って、トップ3のみを再読み込みし、それらすべてを拒否することも可能です。

勝者の名前は、そのターンにおけるエージェントのシステムプロンプトの1行追加に記述される：
...
エージェントは完全なインデックスと独自の判断を保持しており、その1行は最初に参照するエントリを指示するだけである。名簿自体は変更されないため、それに対するプレフィックスキャッシングは依然として有効である。`claude-haiku-4-5-20251001`に対して488件のリクエストを行い、Hermes名簿のスキルを使用した：

| | 間違ったスキルを読み込む | 適合するものがないのに1つを読み込む |
| ------------------------------------ | --------------------- | --------------------------- |
| ロスターのみを持つエージェント alone | 16.8% | 9.8% |
| **TypeSafeによる提案付きのエージェント** | **7.3%** | **4.0%** |
| 正解を渡されたエージェント | 2.5% | 1.2% |

3段目の行は、ミスをするための床がゼロではないことを示している。なぜなら、適切なスキルを与えられたエージェントでも、必ずしもそれをロードするとは限らず、いかに優れた選択方法であっても、その壁を乗り越えることはできないからである。

`suggest()` 関数は最大で1つのスキル名を返し、`suggestion_block()` はそれをシステムプロンプト用にラップし、上記の表を生成したハネスは、あなたのロスターを指し示す準備ができています。
...
| `N1` | ノウル：そもそもスキルが必要か？ / ・相手の行動に介入するか？ / ・書かれた手順に従うか？ / ・それともただ話すか？ | コール1 - 182のスキルをすべてざっと読む |
| `C2` | コール2 - その3つをちゃんと読む | コール2 - その3つをちゃんと読む |
| `Q2` | 選択：3つのうちどれか？ / 今度は実際の詳細を持って | コール2 - その3つをちゃんと読む |
...
* TypeSafeクライアント、Anthropicクライアント、および共有クックブックヘルパーをインストールします。
* [TypeSafe APIキー](https://console.typesafe.ai/keys)と、測定対象エージェント用のAnthropicキーを設定します。
...
`hermes_roster.json`は、[NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent) (MIT) の182のスキルを、特定の固定コミットに固定して保持しています。各レコードには、スキルの名前とカテゴリ、インデックスが示す説明文、完全な説明文、および`SKILL.md`の冒頭部分が含まれています。

以下のインデックスと、その上のプロンプト内の指示は、Hermesからコピーされたものです。
...
## ステップ2: エージェント自身をスコアリングする

`requests.json`は488件の単一ターンリクエストを保持しており、そのうち315件はちょうど1つのスキルによってカバーされ、残りの173件は何によってもカバーされていません。

カバーされたリクエストは、各スキルの固有の`SKILL.md`からClaude Sonnet 5によって作成されたものであり、ラベルは信頼でき、リクエストはユーザーが送信するものよりも容易です。

173件見つかったものは、すべて推測を罰するために書き込まれた：85件の日常リクエスト、42件の技術的な質問（*モナドとは何かを説明せよ*など）、そして46件は、リストがXとそれ以外何もカバーしていないような、*このリストでMastodonに投稿せよ*といった、リストにない特定のスキルを求めている。

スコアリングはエージェントの最初の応答のみを読み取ります。両方の数値はエラー率であるため、それぞれにおいて低い方がより良いです：

* **誤ったロード**: カバートされたリクエストのうち、最初の `skill_view` 呼び出しが
 対応するスキルではなかった割合。何もロードしないターンはミスとしてカウントされる。
* **不要なロード**: カバートされていないリクエストのうち、エージェントが
 `skill_view` を呼び出した割合。
...
提案は、システムプロンプト内の別ブロックに配置し、リスト内ではなくその後に記述します。これにより、リストのテキストはすべてのターンで同一となり、プレフィックスキャッシュを維持できます。

エージェントは、`skill_view`を含む最小限のツールセットを持ち、これはフリーテキスト名を使用してスキルをロードします。正確なロードのためには、名前がスキルと完全に一致している必要があります。
...
エージェントは、今日の運用方式通り、名簿のみを持って最初に実行される。その2つのエラー率は、残りのクックブックが測定する基準値である。
...
誤った負荷は、偶然が示すよりもはるかに高い確率で、正しいスキルのカテゴリに分類されてしまうため、難題となるのは、いくつかの似通ったものを見分けることです。エージェントはすでに、ほぼ正しい場所を探しています。

## ステップ3：ロースター全体をランク付け

一つのリクエストには、二種類の問いが含まれます：

* **`which`** は、182のスキル名すべてを対象とした [`Choice`](/en/primitives/choice/) 質問であり、各選択肢の基準はインデックスの説明（エージェント自身が受け取るのと同じテキスト）です。その確率がランキングとなります。
* リクエストについて [`Noul`](/en/primitives/noul/) の質問が3つあり、以下に示されます。それぞれが、説明を与えるのではなく行動を起こすことを望んでいるかどうかを異なる方法で尋ねています。`prose_suffices` は逆方向でカウントします。それらの平均値によって、何らかの提案を行うかどうかが決まり、0.30未満の場合は何も提案されません。

両方とも1つのリクエストで送信されるため、ランキングとチェックの処理には往復1回分のコストで済みます。

これら3つは、アクションが望ましいかどうかを尋ねるために記述する。主題に関する質問は、*モナドとは何かを説明する*とスキルを要するリクエストを分離しない。なぜなら、どちらもソフトウェア分野だからである。

`Choice` 1つの質問で、この規模のリストはすんなり収まる。数倍大きくなれば、チャンクに分割して各チャンクをランク付けし、その後、この同じショートリスト工程を勝者たちに対して実行する。
...
Notes.appのリクエストは明確であり、その最上位の選択肢が正しいものです。ランキングが何をやってもMastodonのものは救えません：3つの質問はスキルが求められていることを示しており、アカウントへの投稿はアクションであり、Xへの投稿用のスキルがありMastodon用のスキルがない場合、最も近いスキルが勝つからです。

残るのはデッキだけだ。両者のリーダーは`.pptx`スキルであり、60文字という制約下では、広義のChoice質問において、作成スキルが編集スキルより優位に立つ。これは、デッキの作成に関するリクエストの場合である。

## ステップ4：上位3つを再ランク付け

3つの選択肢は、各スキルの固有の
`SKILL.md`の冒頭を含む完全な説明の余地を残しており、2番目のリクエストはより良い証拠に対して同じ質問を投げかけています：

* **`which`** は、各選択肢の基準としてその長いテキストを用いた、短縮リストに対する`Choice`的な質問です。
* **`fits::{name}`** は、各候補に対して`Noul`の質問を行います：このスキルは、リクエストで求められている具体的なことを実行できるか？ それぞれ個別に回答され、すべてが低い評価になることもあり、最高値が0.30を下回る短縮リストは完全に除外されます。
...
⦇0`.pptx`の2つのスキルは、それぞれが独自のテキストを持ち出すとそれぞれ1回だけ分離する：デッキリクエストはオーサリングスキルに切り替わる。

`fits`のNoulとChoiceはここで意見が分かれています：Noulは編集スキルを高く評価する一方で、Choiceは作成スキルを選びます。これらは異なるものを判断しています。Choiceは*どの*スキルかを決定し、Noulは*そもそも*何か言うべきかどうかを決定します。

Mastodon に関するリクエストは両方のチェックを通過しました：その最良の `fits` noul は 0.30 より上にあるため、レシピは Mastodon に関するリクエストに対して X スキルを提案します。これに似たほとんどのリクエストは検出されます。2 回目のパスでは、広範なランキングが渡したものを拒否することしかできず、ここではそれが 3 つのニアミスでした。

以下の関数が全体レシピです：2つのリクエストと2つの閾値があり、戻り値はスキル名が最大1つです。

自分のロスターを指すには、`hermes_roster.json`を置き換えてください。上記のすべての質問は、そのファイルから `name`、`description`、`description_full`、そして `body` を読み取り、ヘルメスについては何も知りません。
...
488件のリクエストそれぞれについて、エージェントへの呼び出しは3回行われ、各呼び出しは1ターンずつ測定されます。実行の違いは、エージェントに伝えられる内容のみです：
...
3番目は達成不可能です。それは、他の2つが測定される基準となる天井です。

その提案の文言は二つの役割を果たしている。それは、提案は無視してよいことを示しており、より強く押し通せば間違った提案に対してもコンプライアンスが得られるためであり、また、間違った提案は何も提案しないよりも悪いためである。さらに、提案がないターンでも、それを伝える一文が送信されます；何も送信しなければ、ロスター自身の「読み込み側に誤差を許す」という指示に対抗するものがなくなってしまう。
...
この提案は、壊すリクエストよりもはるかに多くのリクエストを修正しますが、エージェントが単独で正しく処理していたいくつかのリクエストを壊してしまいます。ターン前に提示する代償として、何の提案もないよりも、自信に満ちた誤った提案の方が説得力があります。
...
* 誤った負荷は16.8%から7.3%に、不要なものは9.8%から4.0%に減少し、これは切り捨てられたインデックスから推測することと回答を渡されることの間の大部分の差を説明している。
* エージェントが単独で正解していたリクエストの一部も、提案が添付されると誤った結果に戻ることがある。カウントは上記の通り。

エージェントが多数の候補を扱う場合、この形式をコピーしてください。まずは安価な全体ランク付けを行い、その後、2〜3つに焦点を当てて詳細に確認します。いずれのステップも、結果が空になる可能性があります。

## プレイグラウンドで開く

ステップ4のデッキリクエストに対するプレイグラウンドリンクを構築する。各候補の完全な説明と本文の抜粋をその基準として使用する。
...
同じ形状が他の場所でも見られます：
[意図ルーティング](/en/patterns/intent-routing/) はスキルではなくハンドラーへのルーティング用、[信頼度](/en/concepts/confidence/) は2つの閾値の選択用、そして[推測ファンアウト](/en/patterns/fan-out/) はすべての質問を1つのリクエストにまとめる用です。