---
title: "自動車研究機能発見"
description: "TypeSafeの質問を提案し、自由テキストを数値特徴に変換し、モデルの誤差を用いて教師ありCatBoost回帰モデルを改善する自動研究ループを実行します。"
section: cases
order: 110
...
translatedFrom: en
---
*TypeSafeの質問は、自由テキストを教師ありCatBoostモデルの数値特徴に変換します。
それらを見つけるために自動リサーチループを使用します。*

CatBoostは数値のテーブルを必要としており、テイスティングノートは数値のテーブルではありません。このクックブックは、ノートに関する質問からテーブルを構築しており、それらの質問は手書きではありません。LLMが質問を提案し、TypeSafeが各行に対してそれらに回答し、CatBoostが回答に基づいて学習します。自動調査部分は次のステップです：CatBoostは自分が使用した質問とまだ誤っている行を報告し、次の提案呼び出しはそのレポートを読み取り、ループが再度実行されます。
...
スコア回答は2つの列になります：回答が指し示す平均レベルと、その平均周りの広がり具合です。ヌール回答は確率1つなので、1つの列です。

データは2,000件のワインレビュー：テイスティングノートを入力し、批評家の80〜100点スケールによる評価を出力する
RMSEは批評家の点数における予測誤差を測定し、大きな外れ値ほど重みを持ち、低いほど優れている。以下の表のすべての数値は、モデルもループも一度も見たことのない800件のレビューから算出されている。
...
| 1回の提案呼び出しからの18質問、ループなし | 1.87 |
| **ループの5ラウンド後の38質問** | **1.77** |

最後の2行がループです。まだ何も手がかりがない最後の提案呼び出しは1.87に達します。
自身の最悪の予測を4回さらに読み込むと1.77になります。 gain の大部分は最初の呼び出しにあり、その後の4回の呼び出しがどれだけ追加するかは、さらに下で測定されます。


> **ヒント** — このノートブックをさらに進めたり、別の問題に適用したりしたい場合は、[次のステップ](#next-steps)をご覧ください。
...
最初のコードセルは、API呼び出し、エンコーディング、指標、チャートスタイルを含む、実装全体です。このファイルが単独で実行できるようにするためにここにあります。ドキュメントサイトではこれを折りたたんで隠します。最初の読み飛ばしではこれをスキップしてください。レシピはその後から始まります。
...
ループは2,000行のうち同じ1,200行（開発行）を繰り返し読み込み、それらの1,200スコアを予測する際に役立つ質問を保持します。同じ行でスコアリングを行うと、主にループがそれらにどれだけ適合しているかを測定することになるため、他の800行は保持され、最後に一度だけスコアリングされます。

## 2つの質問タイプ

提案された質問は2つの種類いずれかであり、その種類によって返される番号が決まります。

* **`intensity`** は度数に関するあらゆる事象に対して `Score` に変換される。その5つのレベルは以下に示されており、列は平均レベルを示すため、「中等度」と「強く」の間に位置する注釈は、その2つの間に表示される。
* **`presence`** は、障害に名前が付けられているかどうかのような yes/no の事象に対して `Noul` に変換される。列はその確率を示す。
...
答えられる質問は、回答前にフィルタリングされません。1ラウンドのすべての質問は同じリクエストで送信されるため、1つ余分な質問を追加しても追加のリクエストコストはかかりません。10行に1行の行に適用される質問は、提案者が読む60のノートでは無用に見えますが、セット全体では最も有用な列となります。

k-foldとは、開発データ行をk個の部分に分割し、他の部分で学習したモデルを用いて各部分を予測することである。これらの予測は3つの役割を果たす：すべての改訂と削除を評価し、次のラウンドで参照するノートを決定し、提案者に対して、どの質問が役立ったか、そして前ラウンドからどれだけ変化があったかを伝える。
...
`run_loop`は5ラウンドすべてを実行し、ラウンドごとにブロックを出力します。新しい質問はそのまま追加されます：その回答はすでに取得済みであり、その重要性は後で、その質問をする価値があったかどうかを示します。修正または削除は、モデルがすでに使用しているカラムを1つ減らすため、それぞれがまず試されます：変更で再フィッティングし、開発エラーが減少した場合のみそれを保持します。再フィッティングにはAPI呼び出しのコストがかからないため、変更を試みて却下することは無料です。
...
`PROPOSER_TASK`はワインに言及する唯一の文字列であり、`featurize()`は文字列のリストを任意に受け取ります。この簡潔な記述を編集すると提案プロンプトが変更され、プロンプトはキャッシュキーの一部であるため、次の実行では各ラウンドごとにAPIが再度呼び出されます。

リクエスト数は質問数ではなく行数に応じて増える：1ラウンドにつき行数ごとに1リクエストなので、100,000行は1ラウンドで100,000リクエストになる。修正は新しい質問としてカウントされるため、すべての行をもう一度処理するコストがかかる。ワーカープールはゆっくりと増やせ。8でも共有キーのレートリミットに達する可能性がある。

## 質問が認識するもの

5つの保留レビューを、スコア範囲の各四分位ごとに1つずつ用意し、38問中15問に対して評価した。重要度順で上位8問のスコア関連質問と、上位7つのNoulである。

それらの15行は、批評家スコアに対して回答がどのように変動するかによってソートされます。
スコアの上昇とともに回答が増加する質問は先頭に、スコアの上昇とともに回答が減少する質問は区切り線の後に配置されます。したがって、左から右へ、最悪の評価から最高の評価へと進むにつれて、区切り線以上の回答は上昇し、区切り線以下の回答は減少するはずです。
...
その3つ目は、10の品質バンドごとに1行あたり⦇0⦇1つで、「欠陥があるまたは不快」から「深遠な」までをカバーします。10段階なのは、`Score`質問が許容する最大レベルが10であり、11にするとサーバーエラーとして返されるからです。レベル0は80点、レベル9は100点に相当します。このようにバンドをスケールに分散させるだけでは不十分です。なぜなら、質問の内容にはこの出版物のスコールが実際にそのスケールのどこに位置するかを示す情報が含まれていないからです。そのため、すべての回答は開発者スコアに基づいて測定された単一のオフセット分だけシフトされます。このオフセットは行ラベルに表示され、このショートカットがスコアから学習するのはこれだけです。

Spearmanは順位相関であり、1.0は保留されたワインが批評家の順序と完全に一致することを意味します。単語数行はCatBoost独自のテキスト処理であり、調整されたテキスト回帰パイプラインではありません。これらはすべて1つのデータセットと1回のループ実行に基づいています。
...
両方の線は、最初の提案から始まり、各ラウンド終了時の質問セットの誤差をプロットしています。破線はクロスバリデーションされた開発誤差であり、これはすべての受諾および拒否の決定が行われる基準です。実線は、ループが決して参照しない保持された行に対して同じ質問セットをスコアリングしたものです。各点は、ラウンドが閉じた時点でのセットの状態を示すため、質問の修正または削除のみを行ったラウンドでも両方の線は移動します。フィーチャーマップは質問が何を測定しているかを示し、誤差は最初の提案後のラウンドが予測を改善したかどうかを示します。

軸はきわめて狭い：そこでのすべての事象は0.2ポイントの範囲内で起こっており、上記の表からのすべてのショートカットは、その上部から遠く離れた位置にある。開発用ラインは、全体を通してホールドアウトラインよりも上にあり、これは学習データサイズの影響である。各開発用フォールドは、開発用行の5分の4で訓練されるのに対し、ホールドアウト値は1,200件すべてを使用したモデルから得られる。2つのラインは共に変動するため、ループが制御する開発用値は、直接観測されないホールドアウト値を追跡している。見出し下の区間は、ホールドアウト行のリサンプリングから算出されており、これは第1ラウンドから第5ラウンドへの変化が、800行におけるノイズよりも大きいかどうかを示している。
...
ホールドアウトラインは、開発ラインよりもさらに低下している。ラウンド1は、作業するためのフィードバックなしで質問を作成し、その後の4ラウンドはホールドアウト行で0.10ポイントの価値があり、95% CI \[-0.147, -0.050]。

ラウンド5では4つの追加、2つの書き換え、8つの削除が提案され、改善が見られない最初の開発番号が示された。245文字のノートについて尋ねられることには限りがあり、ラウンド5には提案が質問の追加から削除へと傾いていた。
...
`importance share`はCatBoostの機能重要度であり、38の質問すべてが100%になるように正規化されています。これは行数、質問数、または予測精度の割合ではありません。スコア質問は平均値と分散の2つの列を持つため、その2つの列の重要度は、パーセンテージが表示される前に合計されます。`note_overall_tone_positivity`は全体の17.4%を占めています。4行目はnoulです：ノートが単一のブドウ園を名指ししているか、それとも他の他の格付け信号であるかは、yes/noの事実であるため、1つの質問として扱われました。

## 次のステップ

この実行ではループを小さく保ちます。直接の拡張：

* 回答に報酬を支払う前に候補をスクリーニングする。提案された質問自体を状態として扱い、Noul について確認する：ソーステキストから回答可能か、その基準の下で意味が一意か、ほとんどの行に適用できるか、行間で変動するか。4つの条件をすべて高い信頼度でクリアした質問のみを送信する。
* 相関のある特徴量を剪定する。開発用行におけるエンコード済み列間の相関を測定し、ほぼ重複するものをクラスタリングし、各クラスタから最も明確または重要な質問を1つ残す。
* 単純なベースラインを追加する。TF-IDF、文字数、その他の構造的特徴量を単独で比較し、その後、発見された列に追加して、それぞれがどの程度貢献するかを測定する。
* 提案者ファミリを混合する。Anthropic、OpenAI、Google Gemini、およびオープンソースモデルを使用して候補バッチを生成し、それらが TypeSafe に到達する前にマージして重複を削除する。異なるファミリを使用することで、1つの提案者への繰り返し呼び出しよりも探索範囲を広げられる。
* 予測モデルと手法を比較する。線形回帰や弾性ネット回帰、サポートベクター回帰機、ランダムフォレスト、および下流の出力が確率的な場合の再較正を試す。発見された特徴量が CatBoost 以外でも有用かどうかを確認する。
* 埋め込みベースラインを追加する。埋め込みは質問なしでノート数を数百の数値に変換する：`sentence-transformers/all-MiniLM-L6-v2`はローカルで実行され、OpenAI の `text-embedding-3-small` はホストされた呼び出しである。これを発見された列に追加し、それらがカバーしていない情報を埋め込みが持っているかどうかを測定する。
* 検証を展開環境に合わせる。未来を予測する場合は時系列分割を使用し、関連する行を一緒に保持する必要がある場合はグループ分割を使用し、特徴量発見とモデル選択の両方によって手つかずにされた最終テストセットを保持する。
* 頭打ちで停止する。交差検証 RMSE が固定されたラウンド数で改善しなくなった場合、または質問またはリクエストの予算に到達した場合、ループを終了する。
* エージェントの Goal モードでより長い探索を実行する。明示的な指標、予算、停止ルールを与え、より多くのラウンドで提案、評価、および改善を行わせる。
* 安定性を確認する。シードやデータスライス間で発見を繰り返し、1つの分割に依存する重要性を持つものではなく、有用性を維持している質問を保持する。

## プレイグラウンドで開く

この共有リンクには、1つのテイスティングノートと、ループが最終的に導き出したすべての質問が含まれています。
